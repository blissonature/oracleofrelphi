...
(function () {
  ...
  function mobileGeometry(root, state, offsetX = 0, offsetY = 0) {
    ...
    if (!liveBoard || items.length < 10) return null;
    ...
    const stickerHeight = Math.max(
      CELTIC_FALLBACK_STICKER_H,
      ...items.map(item => item.querySelector(':scope > .card-row-position-panel')?.offsetHeight || 0)
    );
  ...
  function boardScale(liveBoard) {
    ...
    return {
      x:Number.isFinite(sx) && Math.abs(sx) > .001 ? sx : 1,
      y:Number.isFinite(sy) && Math.abs(sy) > .001 ? sy : 1
    };
  }

  function fitMobileCeltic(root, state, signature) {
    if (!root || root.dataset.relphiMobileCelticFitting !== signature) return;
    const workspace = root.querySelector('.card-row-workspace');
    const liveBoard = root.querySelector('.card-row-board');
  ...
  function mobileCelticGeometry(root) {
    const state = mobileCelticState(root);
    if (!state) {
      root?.classList.remove('relphi-mobile-celtic-pending', 'relphi-mobile-celtic-ready');
      if (root) {
  ...
  function stickerFullText(sticker) {
    return String(sticker?.textContent || '').replace(/\s+/g, ' ').trim();
  }
  ...
  function closeLabelPopover() {
    document.querySelector('.relphi-position-label-popover')?.remove();
  }
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      #shortListPanel #drawing-board-after-canvas *{translate:none!important}
      ...
      @media(max-width:700px){
        ...
        html body #shortListPanel.relphi-mobile-celtic .card-row-board>.card-row-item>.card-row-position-panel{
          box-sizing:border-box!important;
          left:0!important;right:auto!important;top:0!important;bottom:auto!important;
          width:100%!important;min-width:100%!important;max-width:100%!important;
          height:3.35rem!important;min-height:3.35rem!important;max-height:3.35rem!important;
          margin:0!important;padding:.32rem .38rem!important;
          transform:translateY(-100%)!important;
          font-size:clamp(.64rem,2.35vw,.78rem)!important;line-height:1.04!important;
        ...
        html body #shortListPanel.relphi-mobile-celtic .card-row-board>.card-row-item>.card-row-position-panel.relphi-position-label-truncated{cursor:pointer!important;text-decoration-style:dotted!important}
      }
      ...
      #shortListPanel #drawing-board-post-export #downloadRowJson{position:static!important;inset:auto!important;transform:none!important;translate:none!important}
      ...
      #shortListPanel .relphi-reading-options-drawer #downloadRowJson{display:none!important}
      ...
      #shortListPanel.relphi-celtic-readable .card-row-board>.card-row-item[data-row-index="5"]>.card-row-position-panel{top:0!important;bottom:auto!important;margin:0!important;transform:translateY(-100%)!important}
