...
(function () {
  ...
  function organizeBoardOptions(panel) {
    ...
    const drawSetup = setupGroup(setup, 'draw', 'Draw settings', 'Choose the pack, position stickers, repeats, and reversals before drawing.');

    move(spreadSetup, control('rowPositionLabels'));
    move(readingSetup, control('rowName'));
    move(drawSetup, control('rowDrawScope'));
    const drawToggles = document.createElement('div');
    drawToggles.className = 'board-draw-toggles';
    drawSetup.appendChild(drawToggles);
    move(drawToggles, control('rowPositionStickersQuick'));
    move(drawToggles, control('rowAllowRepeats'));
    move(drawToggles, control('rowAllowReversalsQuick'));
    ...
    if (settingsPanel?.querySelector(':scope > summary')) settingsPanel.querySelector(':scope > summary').textContent = 'Options';
  ...
  function start() {
    ...
    style.textContent += 'html body #shortListPanel .board-draw-toggles{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:.4rem!important;width:100%!important;margin-top:.1rem!important}html body #shortListPanel .board-draw-toggles>label{display:flex!important;align-items:center!important;gap:.35rem!important;min-height:2.35rem!important;width:100%!important;margin:0!important;padding:.4rem .5rem!important;border:1px solid #ded5cd!important;border-radius:8px!important;background:#fbf8f5!important;box-sizing:border-box!important}html body #shortListPanel .board-draw-toggles input[type="checkbox"]{flex:0 0 auto!important;width:1rem!important;height:1rem!important;margin:0!important}@media(max-width:700px){html body #shortListPanel .board-draw-toggles{grid-template-columns:1fr!important}}';
