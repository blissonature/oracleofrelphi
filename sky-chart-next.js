(function(){
  ...
  async function render(){
    const status=document.getElementById('wheelStatus');status.textContent='Resolving canonical masters…';
    ...
    for(let i=0;i<12;i++){
      ...
      const gp=polar((R.zIn+R.zOut)/2,start+15),g=svgEl('g');g.setAttribute('transform',`translate(${gp.x} ${gp.y})`);groups.zodiac.appendChild(g);glyphJobs.push(window.SkyChartNextGlyphs.uncircled(g,SIGNS[i],{size:19,color:'#171717'}));
    }
