(function(){
  ...
  async function render(){
    ...
    const groups={};
    ['bHouses','zodiac','aHouses','ticks','leaders','dots','glyphs'].forEach(k=>{groups[k]=svgEl('g');groups[k].dataset.layer=k;svg.appendChild(groups[k])});
    ...
    const glyphJobs=[];
    for(let i=0;i<12;i++){
      const start=i*30,end=start+30,p=svgEl('path');
      p.setAttribute('d',annularPath(R.zIn,R.zOut,start,end));
      p.setAttribute('fill',COLORS[i]);
      p.setAttribute('fill-opacity','.78');
      p.dataset.interactive='sign';
      p.dataset.sign=SIGNS[i];
      groups.zodiac.appendChild(p);
      line(groups.zodiac,R.zIn,R.zOut,start,'zodiac-cusp');
      const gp=polar((R.zIn+R.zOut)/2,start+15),g=svgEl('g');
      g.setAttribute('transform',`translate(${gp.x} ${gp.y})`);
      groups.zodiac.appendChild(g);
      glyphJobs.push(window.SkyChartNextGlyphs.uncircled(g,SIGNS[i],{size:19,color:'#171717'}));
    }
    ...
    for(let d=0;d<360;d++){
      const major=d%10===0;
      line(groups.ticks,R.bDegree-(major?7:3),R.bDegree+(major?7:3),d,'degree-tick'+(major?' major':''));
      line(groups.ticks,R.aDegree-(major?7:3),R.aDegree+(major?7:3),d,'degree-tick'+(major?' major':''));
    }

    const placeA=layoutPlacements(skies.A,cuspA,[448,492,536]);
    const placeB=layoutPlacements(skies.B,cuspB,[310,260,210]);
    async function drawPlacement(item,skyId){
      const sky=skies[skyId],degreeR=skyId==='A'?R.aDegree:R.bDegree;
      const anchor=polar(degreeR,item.longitude),bubble=polar(item.lane,item.displayLongitude);
      const leader=svgEl('line');leader.setAttribute('x1',bubble.x);leader.setAttribute('y1',bubble.y);leader.setAttribute('x2',anchor.x);leader.setAttribute('y2',anchor.y);leader.setAttribute('stroke',sky.color);leader.setAttribute('class','placement-leader');groups.leaders.appendChild(leader);
      const dot=svgEl('circle');dot.setAttribute('cx',anchor.x);dot.setAttribute('cy',anchor.y);dot.setAttribute('r','4.3');dot.setAttribute('fill',sky.color);dot.setAttribute('class','placement-dot');groups.dots.appendChild(dot);
      const g=svgEl('g');g.setAttribute('transform',`translate(${bubble.x} ${bubble.y})`);g.dataset.interactive='placement';g.dataset.sky=skyId;g.dataset.placement=item.id;g.dataset.house=String(item.house);g.dataset.longitude=String(item.longitude);groups.glyphs.appendChild(g);
      await window.SkyChartNextGlyphs.inscribed(g,item.id,{size:16,color:sky.color});
    }
  ...
  function renderLedgers(cuspA,cuspB){
    function ledger(id,sky,cusps){
      ...
      sky.placements.forEach(([glyph,long])=>{
        ...
        const s=svgEl('svg');s.setAttribute('viewBox','-16 -16 32 32');stage.appendChild(s);
        window.SkyChartNextGlyphs.uncircled(s,glyph,{size:11,color:sky.color});
        const sign=Math.floor(long/30),deg=long-sign*30;
        const label=document.createElement('span');label.textContent=`${window.SkyChartNextGlyphs.entries[glyph][0]} in ${SIGN_NAMES[sign]} ${Math.floor(deg)}°`;
    ...
    ledger('skyALedger',skies.A,cuspA);
    ledger('skyBLedger',skies.B,cuspB);
  }
  function wireInteractions(svg,cuspA,cuspB,system){
    svg.querySelectorAll('[data-interactive]').forEach(node=>{
      node.addEventListener('click',()=>{
        svg.querySelectorAll('[data-interactive]').forEach(n=>n.classList.add('is-dim'));
        ...
        if(node.dataset.interactive==='house'){
          ...
          const placements=skies[sky].placements.filter(([,lon])=>houseFor(lon,cusps)===house).map(([id])=>window.SkyChartNextGlyphs.entries[id][0]);
          ...
          const i=SIGNS.indexOf(node.dataset.sign);
          inspector.innerHTML=`<p class="scn-eyebrow">Shared zodiac</p><h2>${SIGN_NAMES[i]}</h2><p>This sign remains fixed while both rainbow house wheels rotate independently around it.</p>`;
        }else{
          const sky=node.dataset.sky,id=node.dataset.placement,long=Number(node.dataset.longitude),sign=Math.floor(long/30),deg=long-sign*30;
          inspector.innerHTML=`<p class="scn-eyebrow">Placement</p><h2>Sky ${sky} · ${window.SkyChartNextGlyphs.entries[id][0]}</h2><p>${SIGN_NAMES[sign]} ${Math.floor(deg)}° · House ${node.dataset.house}</p>`;
        }
      });
    });
  }
