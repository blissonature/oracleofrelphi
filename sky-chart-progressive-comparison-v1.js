...
(function(){
  ...
  async function render(event){
    ...
    const section=document.createElement('section');section.className='sky-selected-progressive';section.setAttribute('aria-label','Progressive comparison reading');section.innerHTML=`<p class="sky-progressive-hint">Tap a symbol to reveal its name, then tap the name to reveal its meaning. Tap an earlier level to fold back to it.</p><div class="sky-progressive-comparison">${side('A',relation.left)}${relationship(relation)}${side('B',relation.right)}</div>`;old.replaceWith(section);bind(section);await draw(section);
  }
