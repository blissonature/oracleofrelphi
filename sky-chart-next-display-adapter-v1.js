// Sky Chart display adapter: use the Sky Chart Next rainbow house wheel for live Sky A/Sky B data.
(function () {
  'use strict';
  ...
  const R = { bIn:165, bOut:330, zIn:330, zOut:420, aIn:420, aOut:575 };
  ...
  function spreadPlacements(items, lane) {
    ...
    sorted.forEach((item, index) => {
      ...
      if (!index) return;
      while (item.display - sorted[index - 1].display < minimum) item.display += minimum;
    });
    if (sorted.length > 1) {
      const drift = sorted.reduce((sum, item) => sum + (item.display - item.value), 0) / sorted.length;
      sorted.forEach(item => { item.display = norm(item.display - drift); item.lane = lane; });
    } else sorted.forEach(item => { item.lane = lane; });
    return sorted;
  }
  ...
  async function drawPlacement(parent, leaders, item, skyId, exactRadius, displayRadius) {
    ...
    const bubble = window.RelphiGlyphComponent.createBubble(host, item.id, {
      radius:16,
      padding:1.25,
      color:SKY[skyId],
      fill:'#fff',
  ...
  function removeCompetingLayers(container) {
    container.querySelectorAll('svg,.relphi-comparison-lollipop-v1,.relphi-canonical-marker-layer').forEach(node => node.remove());
  }
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .unified-sky-wheel .scn-live-wheel{display:block;width:100%;height:auto;overflow:visible;background:#fffdf8;border-radius:1rem}
  ...
  async function render() {
    ...
    if (!container || !window.RelphiGlyphRegistry || !window.RelphiGlyphComponent?.createBubble) return schedule(150);

    const payloadA = read(KEYS.A);
    const payloadB = read(KEYS.B);
    const itemsA = records(payloadA);
    const itemsB = records(payloadB);
    if (!itemsA.length) return schedule(180);
    ...
    try {
      ...
      svg.append(bHouses, zodiac, aHouses, ticks, leaders, glyphs);
      ...
      for (let index = 0; index < 12; index += 1) {
        ...
        const entry = window.RelphiGlyphRegistry.resolve(SIGNS[index]) || window.RelphiGlyphRegistry.get(SIGNS[index]);
        if (entry?.id) jobs.push(window.RelphiGlyphComponent.createGlyph(host, entry.id, { size:19, color:'#171717' }).ready);
      }

      [R.bIn, R.zIn, R.zOut, R.aOut].forEach(radius => {
        svg.appendChild(svgNode('circle', { cx:CENTER.x, cy:CENTER.y, r:radius, class:'scn-ring' }));
      });

      for (let degree = 0; degree < 360; degree += 1) {
        const length = degree % 10 === 0 ? 12 : degree % 5 === 0 ? 8 : 5;
        const cls = degree % 10 === 0 ? 'scn-degree-tick major' : 'scn-degree-tick';
        line(ticks, R.bOut - length, R.bOut + length, degree, cls);
        line(ticks, R.aIn - length, R.aIn + length, degree, cls);
      }

      spreadPlacements(itemsA, R.aIn + 34).forEach(item => jobs.push(drawPlacement(glyphs, leaders, item, 'A', R.aIn, R.aIn + 34)));
      spreadPlacements(itemsB, R.bOut - 34).forEach(item => jobs.push(drawPlacement(glyphs, leaders, item, 'B', R.bOut, R.bOut - 34)));
      await Promise.all(jobs);

      removeCompetingLayers(container);
  ...
  function start() {
    ...
    new MutationObserver(function (records) {
      ...
      const relevant = records.some(record => Array.from(record.addedNodes || []).some(node =>
        node.nodeType === 1 && (node.matches?.('.unified-sky-wheel,svg') || node.querySelector?.('.unified-sky-wheel,svg'))
      ));
      if (relevant) schedule(120);
    }).observe(document.body, { childList:true, subtree:true });
    window.addEventListener('storage', () => schedule(80));
    window.addEventListener('relphi:extra-points-updated', () => schedule(80));
    window.addEventListener('relphi:house-system-changed', () => schedule(80));
    window.addEventListener('relphi:sky-final-render-ready', () => schedule(80));
  }
