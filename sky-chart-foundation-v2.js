...
function redundantIntraskyPair(left,right){return REDUNDANT_INTRASKY_PAIRS.has([String(left||''),String(right||'')].sort().join('|'))}
function intraskyRelationships(list,slot){
  ...
  for(let i=0;i<list.length;i+=1)for(let j=i+1;j<list.length;j+=1){
    const left=list[i],right=list[j];if(redundantIntraskyPair(left.id,right.id))continue;
    const distance=separation(left.value,right.value);
    aspects.forEach(aspect=>{const relation=harmonic?.relation?.(left,right,aspect,distance,windowValue);if(relation)result.push({...relation,scope:slot==='A'?'intra-a':'intra-b'})});
  }
  return result.sort((a,b)=>a.phaseError-b.phaseError||a.harmonicOrder-b.harmonicOrder||a.orb-b.orb)
}
function aspectLine(layer,relation,leftSky,rightSky,relationIndex,cuspsA,cuspsB){
  const radius=Math.max(1,geometry('A').inner-1),from=polar(radius,relation.left.value),to=polar(radius,relation.right.value),leftCusps=leftSky==='A'?cuspsA:cuspsB,rightCusps=rightSky==='A'?cuspsA:cuspsB;
  const attrs={
  ...
  if(relationIndex!=null)attrs['data-relation-index']=String(relationIndex);
