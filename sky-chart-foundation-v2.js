...
function masterAvailable(entry){return!!window.RelphiGlyphMasterSource?.masterAvailable?.(entry)}
...
function start(){shell();void render(true);window.addEventListener('storage',event=>{if((!event.key||event.key===KEYS.A||event.key===KEYS.B)&&!whereWhenEditing())void render(true)});window.addEventListener('relphi:sky-orb-limit-changed',()=>{if(!whereWhenEditing())void render(true)});window.addEventListener('relphi:sky-where-when-committed',()=>void render(true));window.addEventListener('relphi:saved-sky-loaded',()=>{if(!whereWhenEditing())void render(true)})}
