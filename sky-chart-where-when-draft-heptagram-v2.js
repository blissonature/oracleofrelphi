// Live Where and When moment preview. Renders the form's draft moment before commit
// and links that exact draft moment into Planetary Hours without mutating the Sky.
(function(){
'use strict';
if(!/(^|\/)sky-chart\.html$/.test(location.pathname)||window.__relphiSkyWhereWhenDraftHeptagramV2)return;
...
function installStyle(){
  if(document.getElementById('skyWhereWhenDraftHeptagramStyleV2'))return;
  const node=document.createElement('style');
  node.id='skyWhereWhenDraftHeptagramStyleV2';
  node.textContent=`
    ...
    .sky-where-when-heptagram-slot[data-draft-heptagram-ready="true"]{
      display:grid!important;place-items:center;width:100%;min-height:176px;padding:3px 0 6px;box-sizing:border-box
    }
    .sky-where-when-draft-heptagram{
      display:block;width:min(100%,172px)!important;height:auto!important;max-height:none!important;overflow:visible;margin:0 auto
    }
    .sky-where-when-draft-heptagram .sky-ph-node-label{font-size:16px;font-weight:850}
    .sky-where-when-ph-jump{
      display:inline-flex;align-items:center;justify-content:center;max-width:100%;margin:0 auto;padding:.22rem .35rem;
    ...
    @media(max-width:620px){
      .sky-where-when-heptagram-slot[data-draft-heptagram-ready="true"]{min-height:164px}
      .sky-where-when-draft-heptagram{width:min(100%,160px)!important}
...
function planetaryHours(frame,dayKey){
  ...
  return Array.from({length:24},(_,index)=>{
    ...
    const end=start+(bright?dayLength:nightLength);
    return{ruler:sequence[index],start:new Date(start),end:new Date(end)};
  });
}
...
function planetGroup(key,dayKey,hourKey){
  ...
  const label=svg('text',{x:p.x,y:p.y,class:'sky-ph-node-label'});label.textContent=SYMBOLS[key];group.appendChild(label);
  return group;
}
...
function renderNow(slot){
  ...
  try{
    ...
    target.hidden=true;target.removeAttribute('data-draft-heptagram-ready');console.error('Where/When moment preview failed:',error);
  }
}
function schedule(slot,delay=0){if(!slot||!(slot in timers))return;clearTimeout(timers[slot]);timers[slot]=window.setTimeout(()=>renderNow(slot),delay)}
...
function start(){
  ...
  document.addEventListener('click',event=>{
    ...
    schedule(slot,0);window.setTimeout(()=>schedule(slot,0),80);window.setTimeout(()=>schedule(slot,0),180);
  },true);
  ...
  window.addEventListener('relphi:sky-heptagram-canonical-ready',()=>scheduleOpen(0));
  window.addEventListener('relphi:sky-heptagram-source-ready',()=>scheduleOpen(0));
}
