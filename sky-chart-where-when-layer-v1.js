...
document.addEventListener('click',event=>{const tab=event.target.closest('.sky-where-when-actions [data-ww-action="edit"]');if(!tab)return;const slot=slotFor(tab);if(!slot||!complete(profile(slot)))return;event.preventDefault();event.stopImmediatePropagation();requestConfirmed(slot);requestAnimationFrame(hydrate)},true);
