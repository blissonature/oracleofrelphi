...
function cardMarkup(slot,c){
  ...
  return `<a class="inline-rel-card sky-${slot.toLowerCase()}" data-inline-ledger="${esc(c.id)}" href="${esc(href)}" style="text-decoration:none" aria-label="Open full ${esc(c.title)} Tarot Ledger entry"><small>Sky ${slot}</small><img loading="lazy" decoding="async" src="${esc(c.image)}" alt="${esc(c.title)}"><b>${esc(c.title)}</b></a>`;
}
