...
function rowSky(row,side){const explicit=row.dataset[side==='left'?'leftSky':'rightSky'];if(explicit==='A'||explicit==='B')return explicit;const mode=row.dataset.relationshipMode||'A-B';if(mode==='A-A')return'A';if(mode==='B-B')return'B';return side==='left'?'A':'B'}
function find(slot,id,row,side){for(const[k,x]of source(read(KEYS[slot]))){if(!x||typeof x!=='object'||Array.isArray(x))continue;const e=canonical(k,x),v=lon(x);if(e?.id===id&&Number.isFinite(v)){const h=Number(row.dataset[side==='left'?'leftHouse':'rightHouse']);return{id:e.id,entry:e,value:v,sky:slot,house:Number.isFinite(h)&&h>0?h:null}}}return null}
function relation(row){const leftSky=rowSky(row,'left'),rightSky=rowSky(row,'right'),l=find(leftSky,row.dataset.leftPlacement,row,'left'),r=find(rightSky,row.dataset.rightPlacement,row,'right');if(!l||!r)return null;return{left:l,right:r,aspect:String(row.dataset.aspect||''),orb:Number(row.dataset.sourceOrb||0)}}
...
function warmCardArt(src){const key=String(src||'');if(!key)return Promise.resolve();const existing=cardArtWarmups.get(key);if(existing)return existing.ready;const image=new Image();image.decoding='async';let settle;const loaded=new Promise(resolve=>{settle=resolve});image.addEventListener('load',settle,{once:true});image.addEventListener('error',settle,{once:true});image.src=key;const ready=typeof image.decode==='function'?image.decode().catch(()=>loaded):loaded;cardArtWarmups.set(key,{image,ready});return ready}
function warmCurrentCardArt(){const paths=new Set();['A','B'].forEach(slot=>{for(const[,x]of source(read(KEYS[slot]))){if(!x||typeof x!=='object'||Array.isArray(x))continue;const v=lon(x);if(Number.isFinite(v))paths.add(card({value:v}).image)}});paths.forEach(warmCardArt)}
...
function wheelMarkup(rel){const a=point(rel.left.value),b=point(rel.right.value),leftColor=COLORS[rel.left.sky]||COLORS.A,rightColor=COLORS[rel.right.sky]||COLORS.B;return `<div class="inline-rel-wheel"><div class="inline-rel-wheel-stage"><svg viewBox="0 0 120 120" aria-label="Isolated relationship"><circle cx="60" cy="60" r="48" class="inline-rel-ring"/><line x1="60" y1="60" x2="${a.x}" y2="${a.y}" class="inline-rel-radius" style="stroke:${leftColor}"/><line x1="60" y1="60" x2="${b.x}" y2="${b.y}" class="inline-rel-radius" style="stroke:${rightColor}"/><line x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}" class="inline-rel-aspect"/><g class="inline-rel-point-layer" aria-hidden="true"><circle cx="${a.x}" cy="${a.y}" r="5.4" class="inline-rel-point" style="fill:${leftColor};stroke:${leftColor}"/><circle cx="${b.x}" cy="${b.y}" r="5.4" class="inline-rel-point" style="fill:${rightColor};stroke:${rightColor}"/></g></svg></div><div class="inline-rel-orb"><span style="--orb:${Math.min(1,rel.orb)}"></span><strong>${rel.orb.toFixed(2)}°</strong></div></div>`}
function cardMarkup(slot,c){const href=tarotHref(c.id);return `<a class="inline-rel-card sky-${slot.toLowerCase()}" data-inline-ledger="${esc(c.id)}" href="${esc(href)}" style="text-decoration:none" aria-label="Open full ${esc(c.title)} Tarot Ledger entry"><small>Sky ${slot}</small><span class="inline-rel-card-art"><img loading="eager" decoding="async" src="${esc(c.image)}" alt="${esc(c.title)}"></span><b>${esc(c.title)}</b></a>`}
function contextMarkup(rel){const lp=position(rel.left),rp=position(rel.right);return `<div class="inline-rel-house-context"><span>Sky ${rel.left.sky} · ${esc(lp.sign)} ${esc(lp.label)}${rel.left.house?` · H${rel.left.house}`:''}</span><span>Sky ${rel.right.sky} · ${esc(rp.sign)} ${esc(rp.label)}${rel.right.house?` · H${rel.right.house}`:''}</span></div>`}
...
function clearReveal(row){if(!row)return;delete row.dataset.inlineRevealField;delete row.dataset.inlineRevealLevel;const reveal=row.querySelector(':scope>.inline-rel-detail>.inline-rel-top-reveal');if(reveal){reveal.hidden=true;reveal.textContent='';reveal.removeAttribute('style');delete reveal.dataset.level;delete reveal.dataset.field}}
