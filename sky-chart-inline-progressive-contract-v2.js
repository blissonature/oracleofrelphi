...
function decorateHouseTrigger(row,side){
  ...
  const suffix=` · H${house}`,text=span.textContent||'';if(text.endsWith(suffix))span.textContent=text.slice(0,-suffix.length)+' · ';else span.appendChild(document.createTextNode(' · '));
...
function levelActivate(levelNode){const token=levelNode.closest('[data-inline-progressive-token]');if(!token)return;const stage=Number(token.dataset.inlineProgressiveStage||0),level=levelNode.dataset.inlineProgressiveLevel;if(level==='name'){if(stage===1){setStage(token,2);token.querySelector('[data-inline-progressive-level="referent"]')?.focus({preventScroll:true})}else if(stage===2){setStage(token,1);levelNode.focus({preventScroll:true})}}}
...
function installStyles(){
  if(document.getElementById('skyInlineProgressiveContractV2Styles'))return;document.getElementById('skyInlineProgressiveContractStyles')?.remove();
