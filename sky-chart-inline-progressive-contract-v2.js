// Inline relationship progressive reveal v2: seven fixed-address chains.
...
(function(){
'use strict';
if(!/(^|\/)sky-chart\.html$/.test(location.pathname)||window.__relphiInlineProgressiveContractV2)return;
...
function decorateHouseTrigger(row,side){
  const context=row.querySelector(':scope>.inline-rel-detail>.inline-rel-house-context');if(!context)return;
  const index=side==='left'?0:1,span=context.children[index],house=houseNumber(row,side);if(!span||!house)return;
  const field=`${side}-house`,existing=span.querySelector(`[data-inline-progressive-glyph="${field}"]`);if(existing)return;
  const suffix=` · H${house}`,text=span.textContent||'';if(text.endsWith(suffix))span.textContent=text.slice(0,-suffix.length)+' · ';
  else span.appendChild(document.createTextNode(' · '));
  const trigger=document.createElement('span');trigger.className='inline-rel-house-trigger';trigger.dataset.inlineProgressiveGlyph=field;trigger.setAttribute('role','button');trigger.setAttribute('tabindex','0');trigger.setAttribute('title',`Reveal ${HOUSE_NAMES[house]}`);trigger.textContent=`H${house}`;span.appendChild(trigger);
}
function ensureStrip(row){
  ...
  GLYPH_FIELDS.forEach(([field,selector])=>{const glyph=row.querySelector(selector);if(!glyph)return;glyph.dataset.inlineProgressiveGlyph=field;glyph.setAttribute('title','Reveal name');glyph.setAttribute('role','button');glyph.setAttribute('tabindex','0')});
  decorateHouseTrigger(row,'left');decorateHouseTrigger(row,'right');
  return strip;
}
...
function setStage(token,stage){
  if(!token)return;const next=Math.max(0,Math.min(2,Number(stage)||0)),name=token.querySelector('[data-inline-progressive-level="name"]'),referent=token.querySelector('[data-inline-progressive-level="referent"]');
...
function baseActivate(row,field){const token=tokenFor(row,field);if(!token)return;const stage=Number(token.dataset.inlineProgressiveStage||0);setStage(token,stage===0?1:0);if(stage===0)token.querySelector('[data-inline-progressive-level="name"]')?.focus({preventScroll:true})}
function levelActivate(levelNode){
  const token=levelNode.closest('[data-inline-progressive-token]');if(!token)return;const stage=Number(token.dataset.inlineProgressiveStage||0),level=levelNode.dataset.inlineProgressiveLevel;
  if(level==='name'){if(stage===1){setStage(token,2);token.querySelector('[data-inline-progressive-level="referent"]')?.focus({preventScroll:true})}else if(stage===2){setStage(token,1);levelNode.focus({preventScroll:true})}}
}
function expandedRowFor(target){return target?.closest?.('.sky-foundation-relationship-row.is-inline-expanded')||null}
function fieldFromBase(target,row){
  ...
  for(const [field,selector] of GLYPH_FIELDS){const glyph=target.closest?.(selector);if(glyph&&row.contains(glyph))return field}
  return'';
}
function ownRevealEvent(event){
  const row=expandedRowFor(event.target);if(!row)return false;ensureStrip(row);
  const level=event.target.closest?.('[data-inline-progressive-level]');if(level){event.preventDefault();event.stopImmediatePropagation();levelActivate(level);return true}
  const field=fieldFromBase(event.target,row);if(field){event.preventDefault();event.stopImmediatePropagation();baseActivate(row,field);return true}
  return false;
}
...
function installStyles(){
  if(document.getElementById('skyInlineProgressiveContractV2Styles'))return;
  document.getElementById('skyInlineProgressiveContractStyles')?.remove();
  const style=document.createElement('style');style.id='skyInlineProgressiveContractV2Styles';style.textContent=`
    ...
    .inline-rel-house-trigger{display:inline-block;padding:1px 4px;border-radius:5px;font-weight:900;cursor:pointer}.inline-rel-house-trigger:hover{background:rgba(45,39,34,.07)}.inline-rel-house-trigger:focus-visible{outline:2px solid currentColor;outline-offset:2px}
...
function startObserver(){
  installStyles();const list=document.getElementById('skyFoundationRelationshipList');
  if(list)new MutationObserver(()=>requestAnimationFrame(decorateExisting)).observe(list,{childList:true,subtree:true});
  window.addEventListener('relphi:sky-foundation-ready',()=>requestAnimationFrame(decorateExisting));requestAnimationFrame(decorateExisting);
}
// Capture ownership is installed immediately, before the inline row-toggle renderer.
document.addEventListener('click',handleClick,true);document.addEventListener('keydown',handleKey,true);
document.readyState==='loading'?document.addEventListener('DOMContentLoaded',startObserver,{once:true}):startObserver();
})();