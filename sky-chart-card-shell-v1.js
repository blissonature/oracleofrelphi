...
function disclosureMark(){return '<svg class="sky-where-when-disclosure-mark" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M5 18.5 6.2 14 15.6 4.6a2.1 2.1 0 0 1 3 0l.8.8a2.1 2.1 0 0 1 0 3L10 17.8 5 18.5Zm2.4-4.1 2.2 2.2"/></svg>'}
...
function markup(slot,hasProfile){const initial='placements';return `<div class="sky-card-drawers" data-sky-card-drawers="${slot}" data-sky-initial-drawer="${initial}"><details class="sky-card-drawer" data-sky-drawer="where"${initial==='where'?' open':''}><summary class="sky-card-drawer-summary"><span>Where and When</span>${fingerprint(slot,'where','Where and When fingerprint')}${chevron()}</summary><div class="sky-card-drawer-body" data-sky-drawer-mount="where"><section class="sky-where-when-summary" data-sky-where-summary="${slot}"${hasProfile?'':' hidden'}><a class="sky-ph-jump" data-sky-heptagram-frame="${slot}" href="planetaryhours.html" aria-label="Open this Sky in Planetary Hours"><svg class="sky-ph-heptagram" data-sky-heptagram="${slot}" data-canonical-source-ready="pending" viewBox="8 8 344 344" role="img" aria-label="Planetary Hours heptagram for Sky ${slot}"></svg></a><button class="sky-where-when-disclosure" type="button" data-ww-disclosure="${slot}" aria-expanded="false" aria-controls="skyWhereWhenEditor${slot}" aria-label="Show exactly where and when" title="Show exactly where and when">${disclosureMark()}</button></section><div class="sky-where-when-editor-mount" id="skyWhereWhenEditor${slot}" data-ww-editor-mount="${slot}" hidden></div></div></details><details class="sky-card-drawer" data-sky-drawer="placements"${initial==='placements'?' open':''}><summary class="sky-card-drawer-summary"><span>Placements</span>${fingerprint(slot,'placements','Placements fingerprint')}${chevron()}</summary><div class="sky-card-drawer-body sky-where-when-placement-view" data-sky-drawer-mount="placements"></div></details><details class="sky-card-drawer" data-sky-drawer="card-hits"><summary class="sky-card-drawer-summary"><span>Card Hits</span>${fingerprint(slot,'card-hits','Card Hits fingerprint')}${chevron()}</summary><div class="sky-card-drawer-body" data-sky-drawer-mount="card-hits"></div></details></div>`}
...
function setEditorExpanded(slot,expanded){const current=refs(slot);if(!current)return;current.editor.hidden=!expanded;current.disclosure.setAttribute('aria-expanded',expanded?'true':'false');current.disclosure.setAttribute('aria-label',expanded?'Hide exact where and when':'Show exactly where and when');current.disclosure.title=expanded?'Hide exact where and when':'Show exactly where and when';current.summary?.classList.toggle('is-editor-expanded',!!expanded)}
