...
(function(){
  ...
  function draw(parent,id,options){
    const registry=window.RelphiGlyphRegistry,component=window.RelphiGlyphComponent,entry=registry?.get(id)||registry?.resolve(id);if(!entry||!component?.mount)throw new Error('Approved registry glyph unavailable: '+id);return component.mount(parent,entry.id,options);
  }
  function makeGlyphHost(label){const node=document.createElement('span');node.className='sky-foundation-relationship-glyph';node.setAttribute('aria-label',label);return node}
  async function renderRows(relations){
    ...
    relations.forEach((relation,index)=>{
      ...
      try{draw(leftGlyph,relation.left.id,{size:30,circle:false,color:SKY.A});draw(aspectGlyph,relation.aspect.id,{size:30,circle:false,color:relation.aspect.color});draw(rightGlyph,relation.right.id,{size:30,circle:false,color:SKY.B})}catch(error){row.dataset.missingCanonicalGlyph='true';console.error(error)}
    });
