...
assert.match(workflowUi, /> Show position stickers/);
assert.match(workflowUi, /What would you like to know\?/);
...
assert.match(workflowUi, /function readingOptionsResetState\(panel\)/);
assert.match(workflowUi, /function readingOptionsAutoCloseReady\(panel\)/);
assert.match(workflowUi, /hasCards \|\| \(!!template && template !== '__new__'\)/);
...
assert.match(workflowUi, /setReadingOptionsOpen\(panel, true, 'auto-reset'\)/);
assert.match(workflowUi, /panel\.dataset\.relphiReadingOptionsMode === 'auto-reset'/);
