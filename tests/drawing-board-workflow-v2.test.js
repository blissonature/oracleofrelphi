...
assert.match(nav, /drawing-board-interactions-v1\.js\?v=6/);
assert.match(nav, /drawing-board-spread-prefabs-v1\.js\?v=12/);
assert.match(page, /navloader\.js\?v=52/);
...
assert.doesNotMatch(workflow, /function beginDescriptionSelection/);
assert.doesNotMatch(workflow, /descriptionSelecting/);
assert.doesNotMatch(workflow, /document\.addEventListener\('selectstart'/);
assert.match(workflow, /or-card-layer\.relphi-info-layer[\s\S]*display:none!important/);
assert.match(interactions, /navigationCardFromEvent/);
assert.match(interactions, /relphi-ledger-card-link/);
assert.match(interactions, /card-row-board \[data-row-card\]/);
assert.match(interactions, /command\.value = identity\.title/);
assert.match(interactions, /scrollIntoView\(\{ behavior:'smooth', block:'center' \}\)/);
assert.match(interactions, /or-card-layer\.relphi-info-layer[\s\S]*display:none!important/);
...
assert.match(workflow, /Draw settings', 'Choose the pack, reversals, and repeats before drawing/);
