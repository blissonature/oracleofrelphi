...
assert.match(nav, /drawing-board-workflow-v2\\.js\\?v=38/);
...
assert.match(page, /navloader\\.js\\?v=68/);
...
assert.match(workflow, /relphi-reading-options-hotzone/);
assert.match(workflow, /is-reading-options-idle/);
assert.match(workflow, /setOpen\\(false\\)/);
assert.match(workflow, /1000/);
assert.match(workflow, /summary\\.addEventListener\\('mouseenter', openFromEdge\\)/);
assert.match(workflow, /hotzone\\.addEventListener\\('mouseenter', openFromEdge\\)/);
assert.match(workflow, /Show Reading Options · labels and templates/);
