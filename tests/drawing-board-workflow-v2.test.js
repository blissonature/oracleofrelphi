...
assert.match(css, /card-row-card \.or-card-art\.relphi-surface-face\{[^}]*aspect-ratio:auto!important[^}]*border:0!important[^}]*border-radius:0!important[^}]*clip-path:none!important/);
assert.match(sharedStyle, /site-wide tarot source-art integrity/);
assert.match(sharedStyle, /img\[src\]\[src\*="assets\/tarot\/rws\/"\][^{]*\{[^}]*border:\s*0\s*!important[^}]*border-radius:\s*0\s*!important[^}]*clip-path:\s*none\s*!important/s);
assert.match(app, /\.export-card img\{[^}]*border:0[^}]*border-radius:0[^}]*clip-path:none[^}]*object-fit:contain\}/);
assert.match(app, /\.table-art\{[^}]*border:0[^}]*border-radius:0[^}]*clip-path:none[^}]*object-fit:contain\}/);

assert.match(app, /const RELPHI_EXPORT_BRAND = Object\.freeze\(\{/);
assert.match(app, /name:'Oracle of Relphi'/);
assert.match(app, /logo:'logo\.png'/);
assert.doesNotMatch(app, /designation:'an Oracle of Relphi tool'/);
