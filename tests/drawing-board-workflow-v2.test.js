...
assert.match(workflow, /drawSettingsRow\.className = 'board-draw-settings-row'/);
assert.match(workflow, /packControl\.classList\.remove\('relphi-fixed-full-pack'\)/);
...
assert.match(workflow, /button\.setAttribute\('aria-controls', 'drawingBoardReadingOptions'\)/);
...
assert.match(workflow, /relphi-fixed-full-pack\{display:block!important\}/);
