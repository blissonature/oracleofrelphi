...
assert.match(source, /function interceptSelectedTemplatePlaceholder/);
assert.match(source, /templateMode === 'existing' \? prefabById\(selectedId\) : null/);
assert.match(source, /beginDesign\(selected, \{ copy:true \}\)/);
assert.match(source, /document\.querySelector\('#shortListPanel #addCardPlaceholder'\)\?\.click\(\)/);
