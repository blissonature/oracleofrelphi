...
const foundationCss=readFileSync(new URL('../sky-chart-foundation-interactions-v1.css',import.meta.url),'utf8');
...
const relationshipLimit=readFileSync(new URL('../sky-chart-relationship-limit-v1.js',import.meta.url),'utf8');
const relationshipScope=readFileSync(new URL('../sky-chart-relationship-scope-sections-v1.js',import.meta.url),'utf8');
const relationshipExport=readFileSync(new URL('../sky-chart-relationship-export-columns-v1.js',import.meta.url),'utf8');
new Function(vocab);
...
new Function(relationshipLimit);
new Function(relationshipScope);
new Function(relationshipExport);

assert.match(html,/sky-chart-vocab-tab-v1\.js\?v=92/,'Sky Chart must load the Vocab subtab');
assert.match(html,/sky-chart-placement-multiselect-v4\.js\?v=9/,'Sky Chart must load the shared Relationships Placements controller');
assert.match(html,/sky-chart-harmonic-orb-v1\.js\?v=8/,'Sky Chart must load the shared Harmonic Window model fix');
assert.match(html,/sky-chart-orb-control-v1\.js\?v=22/,'Relationships must load the Harmonic Window controller with stable match-count semantics');
assert.match(html,/sky-chart-relationship-limit-v1\.js\?v=5/,'Relationships must load the progressive-proof header result-limit owner after sorting');
...
assert.match(html,/sky-chart-relationship-export-columns-v1\.js\?v=15/,'Relationships export must load the header-Limit-aware exporter');
assert.match(html,/sky-chart-foundation-interactions-v1\.css\?v=16/,'Relationships must load the progressive-proof match-count styling');
assert.match(html,/sky-chart-foundation-interactions-v2\.js\?v=23/,'Sky Chart must load the interaction owner exposing native wheel isolation');
assert.match(html,/sky-chart-card-drawers-v1\.css\?v=15/,'Sky Chart must load the microheptagram sizing fix');
assert.match(html,/sky-chart-filter-control-unified-v1\.css\?v=10/,'Sky Chart must load the shared Vocab/Relationships control styling');
assert.match(html,/sky-chart-zodiac-filter-v1\.css\?v=4/,'Sky Chart must load the shared Vocab/Relationships zodiac styling');
...
assert.match(vocab,/const inlineGlyphCache=new Map\(\)/,'Expanded Vocab glyphs must cache only their fitted canonical state.');
assert.match(vocab,/svg\.dataset\.vocabInlineGlyphReady='false';[\s\S]*svg\.style\.visibility='hidden';[\s\S]*holder\.appendChild\(svg\);[\s\S]*renderInlineGlyph\(svg,entry\.id,fallback\)/,'Expanded Vocab glyph SVGs must enter the DOM hidden at final geometry before canonical fitting begins.');
assert.match(vocab,/sky-vocab-glyph\.has-svg-glyph>svg\[data-vocab-inline-glyph-ready="false"\]\{visibility:hidden!important\}/,'CSS must make an unresolved inline Vocab glyph impossible to paint.');
assert.match(vocab,/svg\.dataset\.vocabInlineGlyphReady='true';[\s\S]*if\(svg\.isConnected\)svg\.style\.visibility='visible'/,'Expanded Vocab glyphs may become visible only after their fitted state is committed.');
...
assert.match(relationshipExport,/Current relationship export/,'A capped export must not describe itself as a complete unfiltered export');
assert.match(relationshipExport,/rows\.length\+' shown'/,'The exported snapshot must report the capped rows actually included, not the larger match count');
...
assert.match(vocab,/createVocabGroup\(container,slot,'structures-sign-polarities','Sign polarities'/,'Sign polarities must be an explicit collapsed Structures group');
assert.match(vocab,/createVocabGroup\(container,slot,'structures-house-polarities','House polarities'/,'House polarities must be an explicit collapsed Structures group');
assert.match(vocab,/function createVocabGroup\(container,slot,key,label,renderBody\)/,'Vocab results must be organized into independently collapsible groups');
assert.match(vocab,/details\.open=state\[slot\]\?\.\[key\]===true/,'Vocab groups must default closed unless the user opened that group previously');
assert.match(vocab,/function populateVocabGroupPreview\(summary,body\)/,'Collapsed Vocab groups must expose a compact visual preview');
assert.match(vocab,/body\.querySelectorAll\('\.sky-vocab-token\[data-vocab-kind="placement"\]'\)/,'Group previews must derive placement glyphs from the same rendered group members');
assert.match(vocab,/groupPreviewGlyph\(tokenNode\)/,'Group previews must include canonical placement glyphs');
assert.match(vocab,/--vocab-polarity-signs','--vocab-concentration-signs','--vocab-placement-signs'/,'Collapsed sign previews must reuse the exact sign rail data from expanded cards');
assert.match(vocab,/--vocab-polarity-houses','--vocab-concentration-houses','--vocab-placement-houses'/,'Collapsed house previews must reuse the exact house rail data from expanded cards');
assert.match(vocab,/horizontalPreviewGradient\(value\)/,'Collapsed rails may change orientation but must preserve the expanded rail color data');
assert.match(vocab,/sky-vocab-group-preview-rail-house\{opacity:\.5\}/,'Collapsed house preview must retain the same 50% opacity language as house rails');
assert.match(vocab,/createVocabGroup\(container,slot,'placements-nodes',CATEGORY_LABELS\.nodes/,'Placement results must also collapse by semantic group');
assert.match(vocab,/createVocabGroup\(container,slot,'placements-axes',CATEGORY_LABELS\.axes/,'Axes must be their own collapsed placement group');
