...
assert.match(html,/sky-chart-orb-control-v1\.js\?v=18/,'Relationships must load the Harmonic Window controller with continuation helper');
assert.match(html,/sky-chart-relationship-limit-v1\.js\?v=1/,'Relationships must load the shared result-limit owner after sorting');
...
assert.match(html,/sky-chart-relationship-export-columns-v1\.js\?v=13/,'Relationships export must load the result-limit-aware exporter');
assert.match(html,/sky-chart-foundation-interactions-v1\.css\?v=4/,'Relationships must load the shown-count and Harmonic continuation styling');
...
assert.match(orbControl,/count\.dataset\.countLabel='shown'/,'Relationship status must identify its fraction as shown results');
assert.match(foundationCss,/#skyFoundationRelationshipCount\[data-count-label="shown"\]::after\s*\{[\s\S]*content:"shown"/,'The relationship count must visibly append shown');
...
assert.match(relationshipExport,/limit=b\.querySelector\('\[data-relationship-limit\]'\)/,'Relationship export summary must include an active result cap');
