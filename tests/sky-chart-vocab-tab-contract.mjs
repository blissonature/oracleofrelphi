...
assert.match(html,/sky-chart-orb-control-v1\.js\?v=19/,'Relationships must load the Harmonic Window controller with match-count semantics');
assert.match(html,/sky-chart-relationship-limit-v1\.js\?v=2/,'Relationships must load the header-mounted shared result-limit owner after sorting');
...
assert.match(html,/sky-chart-foundation-interactions-v1\.css\?v=5/,'Relationships must load the match-count and Harmonic continuation styling');
...
assert.match(relationshipLimit,/const next=String\(eligible\.length\)/,'The Relationships header count must report all current matches before the presentation cap');
