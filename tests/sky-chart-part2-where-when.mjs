...
assert.equal(await page.locator('#skyFoundationA .sky-foundation-row').count(), 10);
...
assert.equal(Object.keys(saved.placements).filter(name => !['Ascendant','Midheaven'].includes(name)).length, 10);
...
assert.equal(await jump.locator('.sky-ph-node-glyph').count(), 7);
