...
(async()=>{
  ...
  try{
    ...
    assert.equal((await page.locator('#drawRandomRowCard').textContent()).trim(),'Attune','See What Surfaces must replace the ordinary Draw affordance with Attune');
    const surfaceLayout=await page.evaluate(()=>window.RelphiDrawingBoardPrefabsBridge.getState().activeLayout);
    assert.equal(surfaceLayout?.id,'see-what-surfaces-active','See What Surfaces should persist as an explicit board mode');
    ...
    assert.equal(await page.locator('#shortListPanel .card-row-board [data-row-card]').count(),0,'Attune must never draw a card before the sacred reading screen opens');
    ...
    await page.waitForSelector('.relphi-attune-reader',{state:'visible'});
    assert.equal(await page.locator('#shortListPanel .card-row-board [data-row-card]').count(),0,'Clicking an empty Surface position must also enter Attune without drawing a card');
