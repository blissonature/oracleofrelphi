...
test('only the static preview and two unreferenced v1 modules consume the package outside review tooling',()=>{const consumers=[];for(const rel of tracked.filter(x=>/\.(?:html|m?js)$/.test(x)&&!x.startsWith('review/')&&!x.startsWith('tests/')&&!x.startsWith('tools/')&&!x.startsWith('assets/'))){const source=fs.readFileSync(path.join(root,rel),'utf8');if(source.includes('assets/canonical-glyphs/v1'))consumers.push(rel);}assert.deepEqual(consumers,['canonical-glyphs-v1-preview.html','relphi-canonical-glyph-element-v1.js','relphi-canonical-glyph-loader-v1.js']);const activeImports=[];for(const rel of tracked.filter(x=>/\.(?:html|m?js)$/.test(x)&&!x.startsWith('review/')&&!x.startsWith('tests/')&&!['relphi-canonical-glyph-loader-v1.js','relphi-canonical-glyph-element-v1.js'].includes(x))){if(/relphi-canonical-glyph-(?:loader|element)-v1\.js/.test(fs.readFileSync(path.join(root,rel),'utf8')))activeImports.push(rel);}assert.deepEqual(activeImports,[]);});
