...
test('static v1 preview is the only current non-review package consumer',()=>{const consumers=[];for(const rel of tracked.filter(x=>/\.(?:html|m?js)$/.test(x)&&!x.startsWith('review/')&&!x.startsWith('tests/')&&!x.startsWith('tools/')&&!x.startsWith('assets/'))){const source=fs.readFileSync(path.join(root,rel),'utf8');if(source.includes('assets/canonical-glyphs/v1'))consumers.push(rel);}assert.deepEqual(consumers,['canonical-glyphs-v1-preview.html']);});
test('blocked identities have no path, hash, Unicode, font, or shared fallback',()=>{const blocked=manifest.identities.filter(x=>!x.candidate_path);assert.equal(blocked.length,60);for(const entry of blocked){assert.equal(entry.candidate_path,null);assert.equal(entry.candidate_sha256,null);assert.ok(!('fallback' in entry));assert.doesNotMatch(JSON.stringify(entry),/unicode|font-family|shared fallback/i);}});
