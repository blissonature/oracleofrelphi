...
async function inspectCeltic(page) {
  return page.evaluate(() => {
    ...
    const union = {
      left:Math.min(...all.map(r=>r.left)), right:Math.max(...all.map(r=>r.right)),
      top:Math.min(...all.map(r=>r.top)), bottom:Math.max(...all.map(r=>r.bottom))
    };
    ...
    return {
      ...
      fitValue:Number(root.querySelector('#rowZoom')?.value || 0)
    };
  });
}
...
function assertTraditional(data) {
  const f = data.faces;
  assert.equal(f.length,10,'Celtic Cross must render ten positions');
  const cx = r => (r.left + r.right) / 2;
  const cy = r => (r.top + r.bottom) / 2;

  assert.ok(cy(f[2]) < cy(f[0]),'3 must be above 1');
  assert.ok(cy(f[3]) > cy(f[0]),'4 must be below 1');
  ...
  for (let i=1;i<staff.length;i++) {
    ...
    assert.ok(upper.bottom <= lower.top + 2 || lower.bottom <= upper.top + 2,'staff cards must not overlap');
  }
  ...
  assert.ok(fill >= .84,`zoom extents wastes too much available space (${(fill*100).toFixed(1)}% fill)`);
  assert.ok(fill <= 1.01,'zoom extents must not overflow the workspace');
}

async function assertResetStable(page) {
  await openOptions(page);
  await page.evaluate(() => {
    window.__relphiResetSamples = [];
    let count = 0;
    function sample() {
      const button = document.querySelector('#drawingBoardOptionsButton');
      const visible = !!button && button.getBoundingClientRect().width > 0 && getComputedStyle(button).visibility !== 'hidden' && getComputedStyle(button).opacity !== '0';
      window.__relphiResetSamples.push(visible);
      if (++count < 18) requestAnimationFrame(sample);
    }
  ...
  await page.waitForFunction(() => {
    ...
    return state && !state.activeLayout && state.slotCount === 0 && !state.hasCards;
  }, { timeout:10000 });
  await page.waitForTimeout(350);
  const result = await page.evaluate(() => ({
    samples:window.__relphiResetSamples || [],
    buttonVisible:!!document.querySelector('#drawingBoardOptionsButton') && document.querySelector('#drawingBoardOptionsButton').getBoundingClientRect().width > 0,
    itemCount:document.querySelectorAll('#shortListPanel .card-row-board>.card-row-item').length,
    ready:document.querySelector('#shortListPanel')?.classList.contains('relphi-drawing-board-ui-ready'),
    stable:document.documentElement.classList.contains('relphi-drawing-board-ui-stable')
  }));
  assert.equal(result.itemCount,0,'Reset Board must leave zero positions');
  assert.equal(result.buttonVisible,true,'Options button must remain visible after Reset Board');
  assert.equal(result.ready,true,'Reset Board must not revoke UI readiness');
  assert.equal(result.stable,true,'Reset Board must preserve stable UI mode');
  assert.ok(result.samples.length >= 8,'reset visibility sampler did not run');
  assert.ok(result.samples.every(Boolean),'Options button disappeared during a painted reset frame');
}
...
(async () => {
  ...
  try {
    await testViewport(browser,{width:1365,height:900},'desktop');
    await testViewport(browser,{width:390,height:844},'mobile');
    console.log('Drawing Board runtime acceptance checks passed.');
  } finally {
    await browser.close();
  }
})().catch(error => {
