...
assert.deepEqual(await preset.locator('option').allTextContents(),['Max','5','10','20','50','Custom…'],'The selector itself must show Max by default, followed by common presets and Custom.');
assert.equal(await preset.inputValue(),'all','Max must be the default uncapped state.');
assert.equal(await preset.locator('xpath=..').locator('.sky-relationship-limit-label').count(),0,'Max must not appear as a separate header label beside the selector.');
