...
assert.ok(progressiveText.startsWith('In Sky A,'));
assert.equal(progressiveText.includes('Between Sky A and Sky B'), false);
assert.ok([' joins ',' gently nudges ',' applies focused pressure to ',' opens a cooperative path to ',' shapes a creative pattern with ',' presses against ',' flows easily with ',' intensifies the pressure on ',' refines an unusual synthesis with ',' keeps adjusting to ',' faces and mirrors '].some(phrase => progressiveText.includes(phrase)));
assert.ok(progressiveText.includes('shows how closely the pattern is aligned'));
assert.ok(progressiveText.includes('The likely transit length is'));
