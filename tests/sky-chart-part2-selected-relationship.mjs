...
assert.equal(await panel.locator('.sky-progressive-token').count(), 10);
assert.deepEqual(await panel.locator('[data-progressive-field]').evaluateAll(nodes => nodes.map(node => node.dataset.progressiveField)), ['A-placement','A-sign','A-degree','A-house','B-placement','B-sign','B-degree','B-house','aspect','orb']);
...
assert.equal(await panel.locator('.sky-progressive-meta > p').count(), 1);
...
assert.equal(await panel.locator('[data-transit-timeline]').count(), 1);
...
const aspectTarget = await page.locator('[data-layer="aspects"]').evaluate(layer => {
  const lines=Array.from(layer.querySelectorAll('line.sky-foundation-aspect:not(.sky-foundation-aspect-hit)')).filter(line=>getComputedStyle(line).display!=='none');
  const segment=line=>{const svg=line.ownerSVGElement,matrix=line.getScreenCTM(),a=svg.createSVGPoint(),b=svg.createSVGPoint();a.x=Number(line.getAttribute('x1'));a.y=Number(line.getAttribute('y1'));b.x=Number(line.getAttribute('x2'));b.y=Number(line.getAttribute('y2'));return{a:a.matrixTransform(matrix),b:b.matrixTransform(matrix)}};
  const distance=(point,part)=>{const dx=part.b.x-part.a.x,dy=part.b.y-part.a.y,length=dx*dx+dy*dy,t=Math.max(0,Math.min(1,((point.x-part.a.x)*dx+(point.y-part.a.y)*dy)/length));return Math.hypot(point.x-(part.a.x+t*dx),point.y-(part.a.y+t*dy))};
  const segments=lines.map(segment);let best=null;
...
assert.equal(await page.locator('.sky-foundation-wheel').getAttribute('class').then(value => value.includes('has-isolation')), true);
assert.equal(await page.locator(`.sky-foundation-aspect[data-relation-index="${aspectIndex}"]:not(.sky-foundation-aspect-hit)`).getAttribute('class').then(value => value.includes('is-selected')), true);
assert.equal(await page.locator('.sky-foundation-placement.is-aspect-endpoint').count(), 2);
assert.equal(Number(await panel.getAttribute('data-relation-index')), listControlledIndex);
...
const progressivePlacement = panel.locator('[data-progressive-field="A-placement"]');
...
assert.equal(await panel.locator('.sky-progressive-token[data-progressive-stage="meaning"]').count(), 10);
