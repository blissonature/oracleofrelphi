...
assert.ok(await page.locator('.sky-foundation-row > svg[data-canonical-ledger-glyph="true"]').evaluateAll(nodes => nodes.every(svg => {
  const box=svg.getBoundingClientRect();
  return Math.abs(box.width-23)<.5 && Math.abs(box.height-23)<.5;
})));
