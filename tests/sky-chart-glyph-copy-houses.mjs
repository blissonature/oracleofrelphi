...
try{
  ...
  await page.waitForFunction(()=>Boolean(window.__relphiCopiedText),null,{timeout:3000});
  const longPressCopiedBeforeRelease=await page.evaluate(()=>window.__relphiCopiedText);
  assert.equal(longPressCopiedBeforeRelease,longPressExpected,'A touch long press must copy while the finger is still down, before iOS can cancel the pointer sequence.');
  ...
  assert.equal(longPressCopiedBeforeRelease.includes('Relationships'),false,'Single-tile long-press copy must not prepend the bulk-copy Relationships heading.');
  ...
  await page.evaluate(()=>{window.__relphiCopiedText=''});
  await longPressRow.dispatchEvent('pointerdown',{pointerId:42,pointerType:'touch',isPrimary:true,button:0,clientX:point.x,clientY:point.y});
  await page.waitForTimeout(120);
  const contextPrevented=await longPressRow.evaluate(async(row,{x,y})=>{
    const event=new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX:x,clientY:y});
    const accepted=row.dispatchEvent(event);
    await new Promise(resolve=>setTimeout(resolve,30));
    return !accepted||event.defaultPrevented;
  },point);
  assert.equal(contextPrevented,true,'The iOS-style contextmenu generated by a hold must be suppressed.');
  await page.waitForFunction(()=>Boolean(window.__relphiCopiedText),null,{timeout:3000});
  assert.equal(await page.evaluate(()=>window.__relphiCopiedText),longPressExpected,'The iOS contextmenu path must also copy only the held relationship tile.');
  await longPressRow.dispatchEvent('pointercancel',{pointerId:42,pointerType:'touch',isPrimary:true,button:0,clientX:point.x,clientY:point.y});
