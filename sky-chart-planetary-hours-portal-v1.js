...
(function () {
  ...
  function planetaryMoment(data) {
    ...
    const next = window.SunCalc.getTimes(localNoon(dayDate.plus({days:1})), data.lat, data.lon);
    const nextSunrise = next.sunrise;
    ...
    const dayRuler = WEEKDAY_RULER[localSunrise.weekday % 7]; // Luxon Sunday=7 -> 0
    const startIndex = CHALDEAN.indexOf(dayRuler);
    const hourRuler = CHALDEAN[(startIndex + ordinal) % 7];
    return { selected:selected, dayRuler:dayRuler, hourRuler:hourRuler, bright:bright, ordinal:ordinal + 1 };
  }
  ...
  function ring(group, radius, word, id) {
    ...
    const arc = el('path',{id:id,d:'M '+(-radius)+' 0 A '+radius+' '+radius+' 0 0 0 '+radius+' 0',fill:'none'});
  ...
  function portalMarkup() {
    return '<a id="relphiPlanetaryHoursPortal" class="relphi-ph-portal" href="planetaryhours.html" aria-label="Open this selected moment in Planetary Hours"><svg viewBox="0 0 360 360" role="img" aria-label="Planetary day and hour heptagram"></svg></a>';
  }
  function ensurePortal() {
    let portal = document.getElementById('relphiPlanetaryHoursPortal');
    if (portal) return portal;
    const center = document.querySelector('#relphiSkyWorkspace .relphi-workspace-center') || document.querySelector('.sky-output-box');
    if (!center) return null;
    const holder = document.createElement('div'); holder.innerHTML = portalMarkup(); portal = holder.firstElementChild;
    center.insertAdjacentElement('afterbegin', portal);
    return portal;
  }
  function linkFor(data) {
    const url = new URL('planetaryhours.html', location.href);
    url.searchParams.set('datetime', data.datetime); url.searchParams.set('lat', data.lat); url.searchParams.set('lon', data.lon);
    url.searchParams.set('tz', data.tz); if (data.loc) url.searchParams.set('loc', data.loc); url.searchParams.set('useSystem','0');
  ...
  async function draw() {
    ...
    if (!data) { portal.hidden = true; return; }
    portal.hidden = false;
    try {
      await dependencies();
      ...
      portal.dataset.slot = activeSlot;
      portal.setAttribute('aria-label','Open '+data.payload.name+' at its selected moment in Planetary Hours');
      portal.addEventListener('click',function(){ try { localStorage.setItem('relphiPlanetaryHoursWhereWhen',JSON.stringify({datetime:data.datetime,lat:data.lat,lon:data.lon,tz:data.tz,loc:data.loc,useSystem:false,savedAt:new Date().toISOString()})); } catch (_) {} },{once:true});
      const svg = portal.querySelector('svg'); svg.replaceChildren();
      const star = el('path',{class:'relphi-ph-heptagram-star',d:(function(){const order=[0,3,6,2,5,1,4,0];return order.map(function(i,n){const p=point(i,116);return (n?'L ':'M ')+p.x.toFixed(2)+' '+p.y.toFixed(2);}).join(' ');})()});
      svg.appendChild(star);
      const ready=[];
      CHALDEAN.forEach(function(id,index){
        const p=point(index,116); const host=el('g',{class:'relphi-ph-node','data-planet':id,transform:'translate('+p.x+' '+p.y+')'});
        svg.appendChild(host);
        const bubble=window.RelphiGlyphComponent?.createBubble(host,id,{radius:15,padding:1,color:'currentColor',fill:'var(--portal-surface)',strokeWidth:2.35});
        if (bubble?.ready) ready.push(bubble.ready);
