...
(function () {
  ...
  function canonicalEntry(key, item) {
    ...
    for (const candidate of candidates) {
      ...
      // Part 1 permits only entries backed by approved SVG assets. No text,
      // Unicode, generated-letter, or generic-symbol fallback is public.
  ...
  function records(payload) {
    ...
    if (omitted.length) console.info('Sky Chart omitted placements without approved SVG assets:', omitted);
  ...
  function annular(inner, outer, start, end) {
    ...
    const a = polar(outer, start), b = polar(outer, start + span), c = polar(inner, start + span), d = polar(inner, start);
  ...
  function radialLine(parent, inner, outer, degree, attrs) {
    const a = polar(inner, degree), b = polar(outer, degree);
  ...
  function shell() {
    ...
    root.innerHTML = `
      <aside id="skyFoundationA" class="sky-foundation-panel" aria-label="Sky A"><header class="sky-foundation-heading"><span class="sky-foundation-slot" style="--slot-color:${SKY.A}">Sky A</span><span class="sky-foundation-name">Sky A</span></header><div class="sky-foundation-body"></div></aside>
      ...
      <aside id="skyFoundationB" class="sky-foundation-panel" aria-label="Sky B"><header class="sky-foundation-heading"><span class="sky-foundation-slot" style="--slot-color:${SKY.B}">Sky B</span><span class="sky-foundation-name">Sky B</span></header><div class="sky-foundation-body"></div></aside>`;
    panel.prepend(root);
  ...
  async function drawBubble(parent, id, options) {
    ...
    const component = window.RelphiGlyphComponent;
    const entry = registry?.get(id) || registry?.resolve(id);
    if (!entry?.asset || !component?.createBubble) throw new Error('Approved SVG asset unavailable: ' + id);
  ...
  function renderCard(slot, payload, list, cusps) {
    ...
    if (!list.length) {
      body.innerHTML = '<p class="sky-foundation-empty">No approved canonical SVG placements are available for this sky.</p>';
      return [];
    }
    ...
    list.forEach(record => {
      ...
      row.innerHTML = `<svg viewBox="-20 -20 40 40" aria-label="${escapeHtml(record.entry.name)}"></svg><span class="sky-foundation-row-name">${escapeHtml(record.entry.name)}</span><span class="sky-foundation-coordinate">${position.text} ${SIGN_NAMES[position.sign]}</span><span class="sky-foundation-house">H${houseFor(record.value, cusps)}</span>`;
      ledger.appendChild(row);
      jobs.push(drawCanonical(row.querySelector('svg'), record.id, { radius:16, padding:1, color:SKY[slot] }).catch(error => {
        row.remove();
        console.error(error);
      }));
    });
  ...
  function spreadPlacements(list, exactRadius, direction) {
    ...
    const result = list.slice().sort((a, b) => a.value - b.value).map((record, index) => ({ ...record, display:record.value, lane:index % 4 === 3 ? laneB : laneA }));
    const gap = 7.5;
  ...
  function buildWheel(listA, listB, cuspsA, cuspsB) {
    const chart = svg('svg', { viewBox:'0 0 1200 1200', role:'img', 'aria-label':'Sky A and Sky B rainbow comparison wheel', class:'sky-foundation-wheel relphi-canonical-ready' });
    ...
    SIGNS.forEach((id, index) => {
      ...
      jobs.push(drawCanonical(host, id, { radius:19, padding:1, color:'#171717' }).catch(error => { host.remove(); console.error(error); }));
    });
    ...
    relationships(listA, listB).forEach(relation => {
      ...
      layers.aspects.appendChild(svg('line', { x1:from.x, y1:from.y, x2:to.x, y2:to.y, stroke:relation.aspect.color, class:'sky-foundation-aspect' }));
    });

    function placements(list, slot, exactRadius, direction, cusps) {
      spreadPlacements(list, exactRadius, direction).forEach(record => {
        const exact = polar(exactRadius, record.value);
        ...
        const host = svg('g', { transform:`translate(${display.x} ${display.y})`, 'data-sky':slot, 'data-placement':record.id, 'data-house':houseFor(record.value, cusps) });
        layers.placements.appendChild(host);
        jobs.push(drawBubble(host, record.id, { radius:16, padding:1, color:SKY[slot], fill:'#fffdf8', strokeWidth:2.35 }).catch(error => { host.remove(); console.error(error); }));
      });
    }
    placements(listA, 'A', R.aDegree, 1, cuspsA);
    placements(listB, 'B', R.bDegree, -1, cuspsB);
  ...
  async function render(force) {
    ...
    const payloadA = read(KEYS.A), payloadB = read(KEYS.B);
    const nextSignature = signature(payloadA, payloadB);
    if (!force && nextSignature === lastSignature) return;
    rendering = true;
    rerender = false;
    try {
      const listA = records(payloadA), listB = records(payloadB);
      const cuspsA = houseCusps(payloadA, listA), cuspsB = houseCusps(payloadB, listB);
      ...
      if (!listA.length || !listB.length) {
        mount.innerHTML = `<p class="sky-foundation-empty">${!listA.length ? 'Sky A needs approved canonical SVG placements.' : 'Sky B needs approved canonical SVG placements.'}</p>`;
      ...
      console.error('Sky Chart Part 1 render failed:', error);
      document.getElementById('skyFoundationWheelMount').innerHTML = '<p class="sky-foundation-empty">The canonical foundation could not render. The browser console identifies the missing approved asset.</p>';
      document.body.classList.remove('sky-foundation-booting');
    } finally {
      rendering = false;
