...
(function(){
  ...
  function canonicalEntry(key,item){
    ...
    for(const candidate of [item?.glyphId,item?.id,item?.name,item?.label,item?.body,item?.planet,item?.point,key]){if(candidate==null)continue;const raw=String(candidate).trim(),entry=registry.resolve(ALIASES[raw.toLowerCase()]||raw)||registry.get(ALIASES[raw.toLowerCase()]||raw);if(entry?.asset||APPROVED_FALLBACKS.has(entry?.id))return entry}
    return null;
  }
  ...
  async function drawCanonical(parent,id,options){const registry=window.RelphiGlyphRegistry,component=window.RelphiGlyphComponent,entry=registry?.get(id)||registry?.resolve(id);if((!entry?.asset&&!APPROVED_FALLBACKS.has(entry?.id))||!component?.draw)throw new Error('Approved glyph unavailable: '+id);return component.draw(parent,entry.id,options)}
  async function drawLedgerCanonical(parent,id,options){
    ...
    if((!entry?.asset&&!APPROVED_FALLBACKS.has(entry?.id))||!component?.draw||!component?.fit)throw new Error('Approved ledger glyph unavailable: '+id);
    const art=await component.draw(parent,entry.id,options);
  ...
  async function drawBubble(parent,id,options){const registry=window.RelphiGlyphRegistry,component=window.RelphiGlyphComponent,entry=registry?.get(id)||registry?.resolve(id);if((!entry?.asset&&!APPROVED_FALLBACKS.has(entry?.id))||!component?.createBubble)throw new Error('Approved glyph unavailable: '+id);const bubble=component.createBubble(parent,entry.id,options);await bubble.ready;return bubble.root}
  ...
  function buildWheel(listA,listB,cuspsA,cuspsB){
    ...
    relationships(listA,listB).forEach(relation=>{const from=polar(R.bIn-1,relation.left.value),to=polar(R.bIn-1,relation.right.value);layers.aspects.appendChild(svg('line',{x1:from.x,y1:from.y,x2:to.x,y2:to.y,stroke:relation.aspect.color,class:'sky-foundation-aspect','data-aspect':relation.aspect.id}))});
  ...
  async function render(force){
    if(rendering){rerender=true;return}const root=shell();if(!root)return;const registry=window.RelphiGlyphRegistry,component=window.RelphiGlyphComponent;if(!registry||!component?.draw||!component?.createBubble){setTimeout(()=>render(true),20);return}
