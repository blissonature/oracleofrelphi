(function(){
  ...
  function relationships(svg){
    const skyA=placementData(svg,'A'),skyB=placementData(svg,'B'),result=[];
  ...
  function annotateExistingPieces(svg){
    const annotateHouseLayer=(layerName,sky)=>{
      ...
      if(!layer)return;
      for(let house=1;house<=12;house++)Array.from(layer.children).slice((house-1)*3,house*3).forEach(node=>{
        node.dataset.focusablePiece='house';node.dataset.sky=sky;node.dataset.house=String(house);
      });
    };
    annotateHouseLayer('aHouses','A');annotateHouseLayer('bHouses','B');

    const zodiac=svg.querySelector('[data-layer="zodiac"]');
    if(zodiac)for(let sign=0;sign<12;sign++)Array.from(zodiac.children).slice(sign*3,sign*3+3).forEach(node=>{
      node.dataset.focusablePiece='sign';node.dataset.signIndex=String(sign);
    });
    ...
    placements.forEach((node,index)=>{
      ...
      if(!leader)return;
      leader.dataset.focusablePiece='placement-leader';leader.dataset.sky=node.dataset.sky;leader.dataset.placement=node.dataset.placement;leader.dataset.longitude=node.dataset.longitude;
    });
  }

  function drawAspectTicks(group){
    const tickGroup=svgEl('g');tickGroup.dataset.layer='combinedAspectTicks';tickGroup.setAttribute('aria-hidden','true');
    for(let degree=0;degree<360;degree++){
      ...
      const outer=polar(ASPECT_RADIUS,degree),inner=polar(ASPECT_RADIUS-length,degree),tick=svgEl('line');
      tick.setAttribute('x1',String(outer.x));tick.setAttribute('y1',String(outer.y));tick.setAttribute('x2',String(inner.x));tick.setAttribute('y2',String(inner.y));tick.setAttribute('class',tickClass);tickGroup.appendChild(tick);
    }
    group.appendChild(tickGroup);
  }

  function drawCombinedSpace(svg){
    const existing=svg.querySelector('[data-layer="combinedAspects"]');if(existing)existing.remove();
    annotateExistingPieces(svg);
    const group=svgEl('g');group.dataset.layer='combinedAspects';group.setAttribute('aria-label','Major cross-sky aspects in one combined central space');
    const chamber=svgEl('circle');
    chamber.setAttribute('cx',String(CENTER.x));chamber.setAttribute('cy',String(CENTER.y));chamber.setAttribute('r',String(ASPECT_RADIUS));
    chamber.setAttribute('fill','rgba(255,253,248,.08)');chamber.setAttribute('stroke','rgba(23,23,23,.24)');chamber.setAttribute('stroke-width','1.4');chamber.setAttribute('vector-effect','non-scaling-stroke');chamber.dataset.clearFocus='true';group.appendChild(chamber);
    ...
    const glyphLayer=svg.querySelector('[data-layer="glyphs"]');if(glyphLayer)svg.insertBefore(group,glyphLayer);else svg.appendChild(group);
  ...
  function clearIsolation(svg){svg.classList.remove('has-isolation');svg.querySelectorAll('.is-kept,.is-selected,.is-dim').forEach(node=>node.classList.remove('is-kept','is-selected','is-dim'));}
  function applyIsolation(svg,target){clearIsolation(svg);const keep=relatedSet(svg,target,'click');if(!keep.size)return;svg.classList.add('has-isolation');keep.forEach(node=>node.classList.add('is-kept'));target.classList.add('is-selected');}
  ...
  function installInteractions(svg){
    if(svg.dataset.combinedInteractionReady==='true')return;svg.dataset.combinedInteractionReady='true';
    svg.addEventListener('pointerover',event=>{const target=event.target.closest('[data-focusable-piece]');if(!target||!svg.contains(target))return;clearHover(svg);relatedSet(svg,target,'hover').forEach(node=>node.classList.add('is-hovered'));},true);
    svg.addEventListener('pointerout',event=>{if(event.relatedTarget&&svg.contains(event.relatedTarget)&&event.relatedTarget.closest('[data-focusable-piece]')===event.target.closest('[data-focusable-piece]'))return;clearHover(svg);},true);
    svg.addEventListener('click',event=>{const target=event.target.closest('[data-interactive]');event.stopImmediatePropagation();if(!target||!svg.contains(target)){clearIsolation(svg);return;}applyIsolation(svg,target);updateInspector(target);},true);
  }

  function refresh(){const svg=document.querySelector('#wheelMount svg.scn-wheel');if(svg)drawCombinedSpace(svg);}
  const mount=document.getElementById('wheelMount');if(mount)new MutationObserver(refresh).observe(mount,{childList:true});
