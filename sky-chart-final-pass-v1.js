...
(function(){
  ...
  const PRESERVE=['Part of Fortune','Vertex','Chiron','North Node','South Node'];
  ...
  function find(source,names){
    const normalized=names.map(name=>name.toLowerCase().replace(/\s+/g,''));
  ...
  function addEditorControls(slot){
    ...
    if(advanced&&!advanced.querySelector('[data-house-system]')){
      const value=read(slot),current=value?.calcProfile?.houseSystem||'whole-sign';
      const label=document.createElement('label');label.className='sky-where-when-label';label.dataset.houseSystem=slot;label.textContent='House system';
      const select=document.createElement('select');select.className='sky-where-when-select';
      Object.entries(HOUSE_SYSTEMS).forEach(([key,name])=>{const option=document.createElement('option');option.value=key;option.textContent=name;option.selected=key===current;select.appendChild(option)});
      const note=document.createElement('p');note.className='sky-house-system-note';note.textContent='Used when this sky is calculated.';
      select.addEventListener('change',()=>{const next=read(slot)||{placements:{}};next.calcProfile={...(next.calcProfile||{}),houseSystem:select.value};save(slot,next,false)});
      label.append(select,note);advanced.prepend(label);
    }
  }
  ...
  function addFilters(){
    const relationshipPanel=document.getElementById('skyFoundationRelationships');if(!relationshipPanel||relationshipPanel.querySelector('.sky-chart-filter-bar'))return;
    const bar=document.createElement('div');bar.className='sky-chart-filter-bar';
    const options=(values)=>values.map(value=>`<option value="${value}">${value==='all'?'All':value}</option>`).join('');
    bar.innerHTML=`<label>Aspect<select data-filter="aspect">${options(['all','conjunction','sextile','square','trine','opposition'])}</select></label><label>Placement<select data-filter="placement"><option value="all">All</option></select></label><label>Sky A house<select data-filter="houseA">${options(['all',...Array.from({length:12},(_,i)=>String(i+1))])}</select></label><label>Sky B house<select data-filter="houseB">${options(['all',...Array.from({length:12},(_,i)=>String(i+1))])}</select></label>`;
    relationshipPanel.insertBefore(bar,relationshipPanel.querySelector('#skyFoundationRelationshipList'));
    bar.addEventListener('change',event=>{if(!event.target.dataset.filter)return;filters[event.target.dataset.filter]=event.target.value;applyFilters()});
  ...
  function refreshPlacementFilter(){
    ...
    const values=new Set();document.querySelectorAll('.sky-foundation-relationship-row').forEach(row=>{if(row.dataset.leftPlacement)values.add(row.dataset.leftPlacement);if(row.dataset.rightPlacement)values.add(row.dataset.rightPlacement)});
    const previous=select.value;select.innerHTML='<option value="all">All</option>';[...values].sort().forEach(value=>{const option=document.createElement('option');option.value=value;option.textContent=value;select.appendChild(option)});select.value=previous||'all';
  }
  ...
  function hideAspectBoxes(){
    document.querySelectorAll('.sky-foundation-aspect-hit, [data-layer="aspects"] rect').forEach(node=>{node.setAttribute('fill','transparent');node.setAttribute('stroke','none');node.style.outline='none'});
  }

  function refresh(){
    queued=false;
    ['A','B'].forEach(slot=>{addHeaderActions(slot);addEditorControls(slot);completeRecord(slot)});
  ...
  function start(){
    const root=document.getElementById('skyFoundationRoot');if(root)observer.observe(root,{childList:true,subtree:true});
