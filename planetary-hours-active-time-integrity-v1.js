...
// Time is the source of the living instrument. In live mode the display never
// reconstructs a clock value from the heptagram slider, and hour 24 retains its
// real fractional progress on the Moon -> Saturn (or equivalent) perimeter edge.
(function () {
  'use strict';
  if (!/(^|\/)planetaryhours\.html$/.test(location.pathname) || window.__relphiPlanetaryHoursActiveTimeIntegrityV1) return;
  window.__relphiPlanetaryHoursActiveTimeIntegrityV1 = true;
  ...
  function is24Hour() {
    return document.getElementById('timeFormatToggle')?.textContent?.trim() === '24h';
  }
  ...
  function isLiveInstrument() {
    return /^Live now\b/i.test(document.getElementById('heptagramHourLabel')?.textContent?.trim() || '');
  }
  ...
  function hour24Fraction(now) {
    ...
    let start = parseWallTime(match[1], dateKey);
    let end = parseWallTime(match[2], dateKey);
    if (!start || !end) return null;
    if (end <= start) end = new Date(end.getTime() + 86400000);
    if (now < start) start = new Date(start.getTime() - 86400000);
    const span = end.getTime() - start.getTime();
    if (!(span > 0)) return null;
    return Math.max(0, Math.min(.999, (now.getTime() - start.getTime()) / span));
  }
  ...
  function line(svg, className, from, to, fraction) {
    ...
    if (!node) {
      ...
      const firstPlanet = svg.querySelector('g[class^="p-"],g[class*=" p-"]');
      svg.insertBefore(node, firstPlanet || null);
    }
    const t = Math.max(0,Math.min(1,fraction));
    node.setAttribute('x1',String(from.x));
    node.setAttribute('y1',String(from.y));
    node.setAttribute('x2',String(from.x + (to.x-from.x)*t));
    node.setAttribute('y2',String(from.y + (to.y-from.y)*t));
    return node;
  }

  function correctHour24(now) {
    const label = document.getElementById('heptagramHourLabel')?.textContent || '';
    if (!/hour\s+24\s+of\s+24/i.test(label)) {
      document.querySelectorAll('.ph-hour24-integrity').forEach(node => node.remove());
      return;
    }
  ...
  function correct() {
    if (!isLiveInstrument()) return;
    const now = activeInstant();
    if (!now) return;
    const panel = document.getElementById('panelCurrentTime');
    if (panel) panel.textContent = formatTime(now);
    correctHour24(now);
  }
