...
const MODES=Object.freeze([{id:'glyphs',label:'Glyphs',level:0},{id:'names',label:'Names',level:1},{id:'referents',label:'Referents',level:2}]);
...
function readMode(){try{const v=localStorage.getItem(STORAGE_KEY);return BY_ID.has(v)?v:'glyphs'}catch(_){return'glyphs'}}
function writeMode(v){const m=BY_ID.has(v)?v:'glyphs';try{localStorage.setItem(STORAGE_KEY,m)}catch(_){}document.documentElement.dataset.relationshipDisplay=m;return m}
...
function syncControl(modeId=readMode()){const owner=control();if(!owner)return;const mode=BY_ID.get(modeId)||MODES[0],value=owner.querySelector('[data-relationship-display-value]');let text=value?.querySelector('[data-relationship-display-value-text]');if(value&&!text){text=document.createElement('span');text.className='sky-relationship-display-value-text';text.dataset.relationshipDisplayValueText='true';value.replaceChildren(text)}if(text&&text.textContent!==mode.label)text.textContent=mode.label;owner.querySelectorAll('[data-relationship-display-option]').forEach(o=>{const selected=o.dataset.relationshipDisplayOption===mode.id;o.setAttribute('aria-selected',selected?'true':'false');o.tabIndex=selected?0:-1});owner.dataset.relationshipDisplayMode=mode.id}
function positionPortal(){const owner=portalOwner,menu=popover(),head=owner?.querySelector('.sky-relationship-display-head');if(!isOpen(owner)||!menu?.classList.contains('is-portaled')||!head)return;const rect=head.getBoundingClientRect(),margin=12,width=Math.min(280,Math.max(230,window.innerWidth-margin*2)),left=Math.min(window.innerWidth-width-margin,Math.max(margin,rect.left+rect.width/2-width/2)),menuHeight=Math.min(150,window.innerHeight-margin*2),roomBelow=window.innerHeight-rect.bottom-margin,above=roomBelow<menuHeight&&rect.top>roomBelow,top=above?Math.max(margin,rect.top-menuHeight-6):Math.min(window.innerHeight-menuHeight-margin,rect.bottom+6);Object.assign(menu.style,{width:`${width}px`,left:`${left}px`,top:`${Math.max(margin,top)}px`})}
function open(owner=control()){const menu=owner?.querySelector('.sky-relationship-display-popover')||popover();if(!owner||!menu)return;portalOwner=owner;owner.classList.add('is-open');menu.hidden=false;menu.classList.add('is-portaled');document.body.appendChild(menu);owner.querySelector('[data-relationship-display-value]')?.setAttribute('aria-expanded','true');owner.querySelector('[data-relationship-display-toggle]')?.setAttribute('aria-expanded','true');requestAnimationFrame(()=>{positionPortal();menu.querySelector('[aria-selected="true"]')?.focus({preventScroll:true})})}
...
function createControl(){const owner=document.createElement('div');owner.className='sky-relationship-display-control';owner.dataset.relationshipDisplayControl='true';const head=document.createElement('div');head.className='sky-relationship-display-head';const label=document.createElement('span');label.className='sky-relationship-display-label';label.textContent='Display';const value=document.createElement('button');value.type='button';value.className='sky-relationship-display-value';value.dataset.relationshipDisplayValue='true';value.setAttribute('aria-label','Choose relationship display');value.setAttribute('aria-haspopup','listbox');value.setAttribute('aria-expanded','false');value.setAttribute('aria-controls','skyRelationshipDisplayPopover');const valueText=document.createElement('span');valueText.className='sky-relationship-display-value-text';valueText.dataset.relationshipDisplayValueText='true';value.appendChild(valueText);const toggle=document.createElement('button');toggle.type='button';toggle.className='sky-relationship-display-toggle';toggle.dataset.relationshipDisplayToggle='true';toggle.setAttribute('aria-label','Choose relationship display');toggle.setAttribute('aria-haspopup','listbox');toggle.setAttribute('aria-expanded','false');toggle.setAttribute('aria-controls','skyRelationshipDisplayPopover');const menu=document.createElement('div');menu.id='skyRelationshipDisplayPopover';menu.className='sky-relationship-display-popover';menu.hidden=true;menu.setAttribute('role','listbox');menu.setAttribute('aria-label','Relationship display');const list=document.createElement('div');list.className='sky-relationship-display-list';for(const mode of MODES){const option=document.createElement('button');option.type='button';option.className='sky-relationship-display-option';option.dataset.relationshipDisplayOption=mode.id;option.setAttribute('role','option');option.textContent=mode.label;option.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();choose(mode.id)});list.appendChild(option)}menu.appendChild(list);head.append(label,value,toggle);owner.append(head,menu);const toggleMenu=()=>isOpen(owner)?close(owner):open(owner);value.addEventListener('click',toggleMenu);toggle.addEventListener('click',toggleMenu);return owner}
function ensureControl(){const bar=document.querySelector('#skyFoundationRelationships .sky-chart-filter-bar');if(!bar)return false;bar.querySelector('[data-relationship-display-control] select')?.closest('[data-relationship-display-control]')?.remove();let owner=bar.querySelector('[data-relationship-display-control]');if(!owner){owner=createControl();bar.appendChild(owner)}syncControl();return true}
function reconcile(){queued=false;installStyles();ensureControl();const root=document.getElementById('skyFoundationRelationships')||document;applyMode(root,false)}
