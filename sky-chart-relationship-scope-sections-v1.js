...
function compactRow(row){const l=String(row.dataset.leftPlacement||''),r=String(row.dataset.rightPlacement||''),a=String(row.dataset.aspect||''),lc=coordinate(row,'left'),rc=coordinate(row,'right'),ls=SIGN_SYMBOLS[Number(row.dataset.leftSign)]||'',rs=SIGN_SYMBOLS[Number(row.dataset.rightSign)]||'',lh=Number(row.dataset.leftHouse),rh=Number(row.dataset.rightHouse),lhText=Number.isInteger(lh)&&lh>=1&&lh<=12?` H${lh}`:'',rhText=Number.isInteger(rh)&&rh>=1&&rh<=12?` H${rh}`:'';if(!l||!r||!lc||!rc)return'';return`${placementSymbol(l)} in ${ls} ${lc}${lhText} ${ASPECT_SYMBOLS[a]||a} ${placementSymbol(r)} in ${rs} ${rc}${rhText}`.replace(/\s+/g,' ').trim()}
