...
(function () {
  ...
  const ASPECTS = [
    { angle:0, orb:3, cls:'is-conjunction' },
    { angle:60, orb:3, cls:'is-sextile' },
    { angle:90, orb:3, cls:'is-square' },
    { angle:120, orb:3, cls:'is-trine' },
    { angle:180, orb:3, cls:'is-opposition' }
  ];
  ...
  function angularDistance(a,b) { const d = Math.abs(normalize(a) - normalize(b)); return Math.min(d,360-d); }
  ...
  function derivedHouse(key, item, payload) {
    ...
    for (let index=0; index<12; index+=1) {
      ...
      const span = normalize(cusps[(index+1)%12] - start) || 30;
      if (normalize(lon - start) <= span) return index + 1;
    }
    return null;
  }
  ...
  function spreadMarkers(entries) {
    ...
    for (let step=1; step<=sorted.length; step+=1) {
      ...
      linear.push({...entry,unwrappedAngle:entry.angle + ((cutAfter + step) >= sorted.length ? 360 : 0)});
    }
    const clusters = [];
    let cluster = [linear[0]];
    for (let i=1; i<linear.length; i+=1) {
      if (linear[i].unwrappedAngle - linear[i-1].unwrappedAngle <= 18) cluster.push(linear[i]);
    ...
    return clusters.flatMap(group => {
      ...
      return group.map((entry,index) => ({...entry,displayAngle:normalize(center + (index - (group.length - 1) / 2) * 12)}));
    });
  }

  function drawInternalAspects(svg, entries, cx, cy, radius) {
    const layer = node('g',{class:'relphi-mini-aspects','pointer-events':'none'});
    svg.appendChild(layer);
    for (let i=0; i<entries.length; i+=1) {
      for (let j=i+1; j<entries.length; j+=1) {
        const a = entries[i], b = entries[j];
        const distance = angularDistance(longitude(a.item),longitude(b.item));
        const aspect = ASPECTS.find(candidate => Math.abs(distance - candidate.angle) <= candidate.orb);
        if (!aspect || aspect.angle === 0) continue;
        const p1 = point(cx,cy,radius,a.angle);
        const p2 = point(cx,cy,radius,b.angle);
        layer.appendChild(node('line',{class:'relphi-mini-aspect ' + aspect.cls,x1:p1.x,y1:p1.y,x2:p2.x,y2:p2.y}));
      }
    }
  }
  ...
  async function drawMiniZodiac(slot, payload, card) {
    ...
    for (let i=0; i<12; i+=1) {
      ...
      const lp = point(cx,cy,79,normalize(180 + i * 30 + 15 - asc));
    ...
    const cusps = cuspArray(payload);
    if (cusps) cusps.forEach((value,index) => {
      const angle = normalize(180 + value - asc);
      const p2 = point(cx,cy,coreR,angle);
      svg.appendChild(node('line',{class:'house-cusp',x1:cx,y1:cy,x2:p2.x,y2:p2.y,stroke:'#77736e','stroke-width':'.62',opacity:'.24'}));
      const span = normalize(cusps[(index+1)%12] - value) || 30;
      const hp = point(cx,cy,48,normalize(180 + value + span/2 - asc));
    ...
    const entries = ordered(map).map(key => ({key,item:map[key],id:resolve(key)})).filter(entry => entry.id && Number.isFinite(longitude(entry.item))).map(entry => ({...entry,angle:normalize(180 + longitude(entry.item) - asc)}));
    drawInternalAspects(svg,entries,cx,cy,coreR - 3);
  ...
  function bind(card, slot, payload) {
    ...
    card.addEventListener('click', event => {
      ...
      if (action === 'copy') window.RelphiGlyphCopySerializer?.copySky(payload).then(() => {
  ...
  function styles() {
    ...
    style.textContent = `
      @media(min-width:1180px){#relphiSkyWorkspace.has-sky-b{grid-template-columns:minmax(138px,158px) minmax(880px,1fr) minmax(138px,158px)!important;gap:9px!important}}
      .relphi-skinny-sky-card{--panel-accent:#dc1f18;color:#111;overflow:hidden}.relphi-skinny-sky-card.is-blue{--panel-accent:#3166e2}
      .relphi-skinny-head{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:.2rem;border-bottom:1px solid #e4e6ea}.relphi-skinny-head .relphi-workspace-tab{padding:.38rem .58rem!important;font-size:.68rem!important}.relphi-skinny-head h2{margin:0;font-size:.78rem;line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.relphi-skinny-actions{display:flex;align-items:center}.relphi-skinny-actions button{border:0;background:transparent;padding:.28rem .18rem;font:700 .6rem system-ui;cursor:pointer}.relphi-skinny-actions button:last-child{font-size:.8rem}
      .relphi-skinny-solo{display:block;width:100%;margin:0;padding:.15rem 0 .22rem;border:0;border-radius:0;background:#fff;box-shadow:none}.relphi-skinny-solo svg{display:block;width:92px;height:auto;margin:0 auto;overflow:visible}
      .relphi-mini-aspect{fill:none;stroke-width:.65;opacity:.32;vector-effect:non-scaling-stroke}.relphi-mini-aspect.is-opposition,.relphi-mini-aspect.is-square{stroke:#d65a55}.relphi-mini-aspect.is-trine,.relphi-mini-aspect.is-sextile{stroke:#6ba96b}
      .relphi-skinny-ledger{display:grid;border-top:1px solid #e4e6ea}.relphi-skinny-row{display:grid;grid-template-columns:15px 15px 52px 24px;justify-content:start;align-items:center;gap:.12rem;min-height:21px;padding:.1rem .24rem;border:0;border-bottom:1px solid #eff0f2;background:#fff;color:inherit;text-align:left;cursor:pointer}.relphi-skinny-row:hover,.relphi-skinny-row:focus-visible{background:color-mix(in srgb,var(--panel-accent) 7%,#fff)}.relphi-skinny-body-glyph,.relphi-skinny-sign-glyph{display:block;width:13px;height:13px;overflow:visible;color:var(--panel-accent)}.relphi-skinny-sign-glyph{color:#111}.relphi-skinny-coordinate,.relphi-skinny-house{font-size:.63rem;font-variant-numeric:tabular-nums;white-space:nowrap}.relphi-skinny-house{text-align:right;color:#666}
      @media(max-width:760px){.relphi-skinny-solo{max-width:160px;margin:auto}.relphi-skinny-solo svg{width:104px}.relphi-skinny-row{grid-template-columns:18px 18px 72px 28px;padding-left:.55rem}.relphi-skinny-coordinate,.relphi-skinny-house{font-size:.72rem}}
    `;
