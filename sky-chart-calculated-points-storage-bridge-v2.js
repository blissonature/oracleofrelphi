...
(function () {
  'use strict';
  if (!/(^|\/)sky-chart\.html$/.test(location.pathname) || window.__relphiCalculatedPointStorageBridgeV5) return;
  window.__relphiCalculatedPointStorageBridgeV5 = true;
  ...
  function profileOf(p) {
    const e = p.calcProfile && typeof p.calcProfile === 'object' ? p.calcProfile : {};
    return Object.assign({}, e, { dateTime:field('skyCalcDateTime') || e.dateTime || '', latitude:field('skyCalcLatitude') || e.latitude || '', longitude:field('skyCalcLongitude') || e.longitude || '', location:field('skyCalcLocation') || e.location || '', timeZone:field('skyCalcTimeZone') || e.timeZone || '', houseSystem:field('skyCalcHouseSystem') || e.houseSystem || 'whole-sign' });
  }
  function dateOf(p) {
  ...
  function augment(p) {
    ...
    p.calcProfile=profileOf(p); const q=p.calcProfile, d=dateOf(q), asc=lonOf(get(m,['Rising','Ascendant','ASC','AC'])), mc=lonOf(get(m,['MC','Midheaven'])), sun=lonOf(get(m,['Sun'])), moon=lonOf(get(m,['Moon']));
    if (d) { const jd=d.getTime()/86400000+2440587.5, node=meanNode(jd); put(m,p,'North Node',node,{retrograde:true,glyph:'☊',calculation:'mean lunar node'},['Node','Mean North Node']); put(m,p,'South Node',norm(node+180),{retrograde:true,glyph:'☋',calculation:'opposite mean lunar node'}); put(m,p,'Lilith',meanLilith(jd),{glyph:'⚸',calculation:'mean lunar apogee'},['Black Moon Lilith','BML']); }
    const eps=Number(q.obliquityDegrees ?? q.obliquity ?? 23.4392911), lat=Number(q.latitude ?? q.lat ?? q.coordinates?.latitude); let lst=Number(q.siderealDegrees ?? q.localSiderealDegrees ?? q.lstDegrees ?? q.localSiderealTimeDegrees); if (!Number.isFinite(lst)&&Number.isFinite(mc)) lst=ra(mc,eps);
    put(m,p,'Vertex',vertex(lst,lat,eps),{glyph:'Vx',calculation:'prime vertical intersection'},['Vx']);
    if ([asc,sun,moon].every(Number.isFinite)) { const h=Number(get(m,['Sun'])?.house), day=Number.isFinite(h)?h>=7&&h<=12:true; put(m,p,'Part of Fortune',norm(day?asc+moon-sun:asc+sun-moon),{glyph:'⊗',calculation:day?'day formula':'night formula'},['Fortune','POF']); }
    return p;
  }
  ...
  function finish(key,p) {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('relphi:extra-points-updated',{detail:{calculated:true,chiron:true,placementCount:Object.keys(m).length}}));
    } catch (e) {
      nativeSet.call(localStorage,key,JSON.stringify(p));
      status('Calculated 19 placements. Chiron setup is in progress: '+e.message,true);
      window.dispatchEvent(new CustomEvent('relphi:extra-points-updated',{detail:{calculated:true,chiron:false,placementCount:Object.keys(m).length}}));
  ...
  Storage.prototype.setItem=function(key,value){
    if(this===localStorage&&KEYS.has(String(key))){
      try { const p=JSON.parse(String(value)), m=mapOf(p); if(Date.now()<activeUntil&&String(key)===activeKey){ if(!m||!all(m,CORE)) return; augment(p); if(!all(m,DERIVED)) return; if(!get(m,['Chiron'])){ finish(String(key),p); return; } } else if(m&&all(m,CORE)) augment(p); value=JSON.stringify(p); } catch(_){}
    }
    return nativeSet.call(this,key,value);
  };
  window.RelphiCalculatedPointStorageBridge={augment,hasCompletePrimarySky:m=>all(m,CORE),calculateChiron:d=>window.RelphiChironLocal.calculate(d)};
})();