(function () {
  ...
  function injectStyles() {
    const oldStyle = document.getElementById('relphi-relationship-color-hints');
    if (oldStyle) oldStyle.remove();
    if (document.getElementById('relphi-relationship-color-hints-pr22')) return;
    const style = document.createElement('style');
    style.id = 'relphi-relationship-color-hints-pr22';
 ...
 `;
  ...
  function addPreviewBadge() {
    if (document.getElementById('relphi-pr22-preview-badge')) return;
    const badge = document.createElement('div');
    badge.id = 'relphi-pr22-preview-badge';
    badge.textContent = 'Previewing PR #22 · Music Theory aspect colors';
    badge.style.cssText = 'position:fixed;right:.75rem;bottom:.75rem;z-index:99999;background:#111;color:#fff;border:2px solid #dc1f18;border-radius:999px;padding:.55rem .8rem;font:700 12px/1.2 Montserrat,Arial,sans-serif;box-shadow:0 4px 18px rgba(0,0,0,.2)';
