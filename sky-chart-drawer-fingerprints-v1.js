// Data-derived fingerprints for collapsed Placements and Card Hits drawers.
(function(){
'use strict';
...
function recordName(key,item){
  return String(item?.name||item?.label||item?.body||item?.planet||item?.point||item?.id||item?.glyphId||key||'').trim();
}
function normalizedId(name){return String(name||'').toLowerCase().replace(/[\s_-]+/g,'')}
function placementRecords(payload){
  return source(payload).map(([key,item])=>{const value=longitude(item);if(!Number.isFinite(value))return null;const name=recordName(key,item),id=normalizedId(name);return{name,id,value,item}}).filter(Boolean);
}
...
function axisValue(records,primaryIds,oppositeIds){
  const primary=records.find(record=>primaryIds.includes(record.id));
  if(primary)return primary.value;
  const opposite=records.find(record=>oppositeIds.includes(record.id));
  return opposite?norm(opposite.value+180):NaN;
}
function addAxis(root,cx,cy,radius,degree,className){
  if(!Number.isFinite(degree))return;
  const a=polar(cx,cy,radius,degree),b=polar(cx,cy,radius,degree+180);
  root.appendChild(svg('line',{x1:a.x,y1:a.y,x2:b.x,y2:b.y,class:className}));
}
...
function renderPlacements(slot,payload){
  ...
  const silhouette=bins.map((count,index)=>{
    const radius=4.7+Math.min(4,count)*2.05;
    return polar(cx,cy,radius,index*30+15);
  });
  if(ordinary.length){
    root.appendChild(svg('polygon',{points:silhouette.map(point=>`${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(' '),fill:skyColor,'fill-opacity':'.20',stroke:skyColor,'stroke-opacity':'.58','stroke-width':'.72','stroke-linejoin':'round'}));
  }
  ordinary.forEach(record=>{
    const point=polar(cx,cy,12.35,record.value);
    root.appendChild(svg('circle',{cx:point.x,cy:point.y,r:ordinary.length>18?1.05:1.3,fill:skyColor,stroke:'#fff','stroke-width':'.62'}));
  });
  mount.appendChild(root);
  const asc=axisValue(records,['asc','ascendant','rising'],['dsc','descendant']);
  const mc=axisValue(records,['mc','midheaven'],['ic','imumcoeli']);
...
function renderCardHits(slot){
  ...
  const api=window.RelphiSkyCardHitsDrawer;
  const hits=api?.getHits?.(slot)||[];
  mount.replaceChildren();
  ...
  shown.forEach(hit=>{
    ...
    image.src=api.thumbnailFor(hit.card,16,28);image.alt='';image.width=16;image.height=28;image.loading='lazy';image.decoding='async';
    card.appendChild(image);
    const chip=document.createElement('span');chip.className='sky-card-hits-fingerprint-count';chip.textContent=String(hit.count);card.appendChild(chip);
...
function renderSlot(slot){const payload=read(slot);renderPlacements(slot,payload);renderCardHits(slot)}
