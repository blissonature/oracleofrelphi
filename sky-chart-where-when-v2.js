...
function calculateSky(slot,selected,date,time){
  const dt=localDateTimeToInstant(date,time,selected.timezone),instant=dt.toUTC().toJSDate(),placements={};
  ...
  delete metadata.liveNowOrigin;delete metadata.liveNowAt;delete metadata.liveNowLatitude;delete metadata.liveNowLongitude;delete metadata.liveNowMigrated;
...
async function submit(slot,form){const selected=cardState[slot].selected,date=form.querySelector('[data-ww-field="date"]')?.value||'',time=form.querySelector('[data-ww-field="time"]')?.value||'';if(!selected)return status(slot,'Choose a canonical location first.',true);if(!date||!time)return status(slot,'Enter both the local date and local time.',true);setBusy(slot,true);status(slot,'Calculating placements and Planetary Hours…');try{const nextPayload=calculateSky(slot,selected,date,time);writeJson(SLOT_KEYS[slot],nextPayload);clearEditor(slot);dispatchSlotChange(slot);finishWhereWhen(slot,true);scheduleSummary(slot,true);requestAnimationFrame(()=>window.RelphiSkyCardShell?.openDrawer?.(slot,'placements'))}catch(error){setBusy(slot,false);status(slot,error.message||'The Sky could not be calculated.',true)}}
...
document.addEventListener('click',event=>{
  ...
  const actionNode=event.target.closest('[data-ww-action]');if(!actionNode)return;const slot=eventSlot(actionNode);if(!slot)return;const action=actionNode.dataset.wwAction;if(action==='cancel')closeEditor(slot);else if(action==='search-location')searchLocation(slot);else if(action==='select-location')selectLocation(slot,actionNode);else if(action==='use-current-location')useCurrentLocation(slot);else if(action==='resolve-coordinates')resolveCoordinates(slot);else if(action==='infer')infer(slot);else if(action==='apply-inference')applyInference(slot)
});
