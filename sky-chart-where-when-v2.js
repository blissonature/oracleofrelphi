...
async function submit(slot,form,options={}){const selected=cardState[slot].selected,date=form.querySelector('[data-ww-field="date"]')?.value||'',time=form.querySelector('[data-ww-field="time"]')?.value||'';if(!selected)return status(slot,'Choose a canonical location first.',true);if(!date||!time)return status(slot,'Enter both the local date and local time.',true);setBusy(slot,true);status(slot,'Calculating placements and Planetary Hours…');try{const nextPayload=calculateSky(slot,selected,date,time,options);writeJson(SLOT_KEYS[slot],nextPayload);if(options.liveOrigin==='use-now'){try{localStorage.setItem(`relphiSkyLiveAgeAnchor${slot}`,JSON.stringify({origin:'use-now',at:nextPayload.metadata.liveNowAt}))}catch(_){}window.dispatchEvent(new CustomEvent('relphi:sky-live-origin-changed',{detail:{slot,origin:'use-now',at:nextPayload.metadata.liveNowAt}}));window.dispatchEvent(new CustomEvent('relphi:sky-name-updated',{detail:{slot,name:'Now',source:'use-now'}}))}clearEditor(slot);dispatchSlotChange(slot);finishWhereWhen(slot,true);scheduleSummary(slot,true);requestAnimationFrame(()=>window.RelphiSkyCardShell?.openDrawer?.(slot,'placements'))}catch(error){setBusy(slot,false);status(slot,error.message||'The Sky could not be calculated.',true)}}
