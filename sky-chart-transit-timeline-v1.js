...
function boundary(center,direction,model,errorAt){const[stepDays,maxSteps]=model.settings,step=stepDays*DAY,fn=ms=>Math.abs(errorAt(ms))-model.limit;let previous=center,previousValue=fn(previous);if(!Number.isFinite(previousValue)||previousValue>0)return null;for(let i=1;i<=maxSteps;i++){const next=center+direction*i*step,value=fn(next);if(!Number.isFinite(value))return null;if(value>0)return root(Math.min(previous,next),Math.max(previous,next),fn);previous=next;previousValue=value}return null}
...
function signature(row,model){return[row.dataset.leftSky||'',row.dataset.leftPlacement,row.dataset.aspect,row.dataset.rightSky||'',row.dataset.rightPlacement,model?.moving||'',model?.date?.toISOString?.()||'',model?.limit||''].join('|')}
function renderMeta(row,model,data){let host=row.querySelector(':scope>.sky-relationship-transit-meta');if(!host){host=document.createElement('span');host.className='sky-relationship-transit-meta';row.appendChild(host)}if(!model||!data){host.remove();return}const motion=data.stationary?(data.retrograde?'Stationing Rx':'Stationing direct'):(data.retrograde?'Retrograde':'Direct'),pass=data.passes?.count>1?`<span><strong>Pass:</strong> ${data.passes.current} of ${data.passes.count}</span>`:'';host.innerHTML=`<span><strong>Active:</strong> ${durationLabel(data.duration)}</span><span class="${data.retrograde?'is-rx':''}"><strong>Motion:</strong> ${motion}</span><span><strong>Phase:</strong> ${data.phase}</span>${pass}`;host.title=`${model.movingRecord.entry.name} is the moving endpoint. Current active orb window: ${durationLabel(data.duration)}.${data.passes?.count>1?` Exact pass ${data.passes.current} of ${data.passes.count}.`:''}`}
