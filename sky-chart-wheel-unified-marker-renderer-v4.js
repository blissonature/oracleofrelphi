// Sky Chart preview: grouped marker renderer with path-only special-point vectors.
(function () {
  'use strict';
  ...
  function vectorGroup(unit, className, cx, cy, color) {
    ...
    g.style.setProperty('stroke',color,'important');
    g.style.setProperty('color',color,'important');
    g.style.setProperty('stroke-linecap','round','important');
    g.style.setProperty('stroke-linejoin','round','important');
    unit.appendChild(g);
    return g;
  }

  function addPath(g, d, color, width) {
    const path=document.createElementNS(NS,'path');
    path.setAttribute('d',d);
    path.style.setProperty('fill','none','important');
    path.style.setProperty('stroke',color,'important');
    path.style.setProperty('stroke-width',String(width),'important');
  ...
  function drawPluto(unit,cx,cy,color) {
    ...
    addPath(g,'M -3.2 -7 A 3.2 3.2 0 1 0 3.2 -7 A 3.2 3.2 0 1 0 -3.2 -7',color,1.9);
    addPath(g,'M -8 -1.8 Q 0 5.7 8 -1.8',color,1.9);
    addPath(g,'M 0 3.4 V 11 M -5 8 H 5',color,1.9);
  }

  function drawFortune(unit,cx,cy,color) {
    const g=vectorGroup(unit,'relphi-fortune-vector',cx,cy,color);
    addPath(g,'M -10.8 0 A 10.8 10.8 0 1 0 10.8 0 A 10.8 10.8 0 1 0 -10.8 0',color,1.85);
    addPath(g,'M -7.6 -7.6 L 7.6 7.6 M 7.6 -7.6 L -7.6 7.6',color,1.85);
  }
