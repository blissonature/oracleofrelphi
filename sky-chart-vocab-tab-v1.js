...
const ORDER=['sun','moon','asc','mercury','venus','mars','jupiter','saturn','uranus','neptune','pluto','north-node','south-node','chiron','lilith','part-of-fortune','vertex','mc','ic','dsc','child','asteroid-lilith','hidalgo','victoria','daphne','vesta'];
...
function ordinaryPlacements(list){
  const skip=new Set(['asc','dsc','mc','ic']);
  return list.filter(record=>!skip.has(record.id));
}
function renderParagraph(slot,panel){
  const list=records(slot),paragraph=panel.querySelector('[data-sky-vocab-paragraph]');
  if(!paragraph)return;
  paragraph.replaceChildren();
  if(!list.length){paragraph.textContent='Add or calculate placements to read this sky as vocabulary.';return}
  const byId=new Map(list.map(record=>[record.id,record])),ic=byId.get('ic'),mc=byId.get('mc'),asc=byId.get('asc'),dsc=byId.get('dsc');
  if(ic&&mc)appendSentence(paragraph,axisSentence(ic,mc));
  if(asc&&dsc)appendSentence(paragraph,axisSentence(asc,dsc));
  ordinaryPlacements(list).forEach(record=>appendSentence(paragraph,phrasePlacement(record)));
  relations(list).forEach(relation=>appendSentence(paragraph,relationSentence(relation)));
...
function controlsMarkup(){
  ...
  return '<div class="sky-vocab-controls" role="group" aria-label="Vocabulary layers">'+
    ['glyphs','names','referents'].map(id=>'<label class="sky-vocab-check"><input type="checkbox" data-vocab-layer="'+id+'" '+(state[id]?'checked':'')+'><span>'+id[0].toUpperCase()+id.slice(1)+'</span></label>').join('')+
    '</div>';
}
function ensurePanel(slot,view){
  ...
  panel.innerHTML=controlsMarkup()+'<p class="sky-vocab-paragraph" data-sky-vocab-paragraph></p>';
  ...
  panel.querySelectorAll('[data-vocab-layer]').forEach(input=>input.addEventListener('change',()=>{
    const state=displayState();state[input.dataset.vocabLayer]=input.checked;saveDisplay(state);
    document.querySelectorAll('[data-sky-vocab-panel]').forEach(other=>{other.querySelectorAll('[data-vocab-layer]').forEach(control=>{control.checked=!!state[control.dataset.vocabLayer]});renderParagraph(other.dataset.skyVocabPanel,other)});
  }));
