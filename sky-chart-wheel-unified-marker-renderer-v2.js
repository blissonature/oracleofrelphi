// Sky Chart preview: authoritative grouped marker renderer.
(function () {
  'use strict';
  ...
  function removeLegacyGlyphLayers(group) {
    group.querySelectorAll('svg.relphi-bold-inline-glyph,svg.relphi-colored-glyph,image.relphi-bubble-glyph-image').forEach(node => node.remove());
  ...
  function removeCompetingPlutoMarks(group, vector) {
    group.querySelectorAll('.chart-wheel-marker-glyph, svg, image, use').forEach(function (node) {
      if (node === vector || vector.contains(node)) return;
      if (node.closest('g.relphi-pluto-vector') === vector) return;
      if (node.classList?.contains('chart-wheel-marker-glyph')) {
        node.textContent = '';
        node.style.setProperty('display', 'none', 'important');
        node.style.setProperty('visibility', 'hidden', 'important');
      } else if (!node.closest('g.relphi-marker-unit') || node.closest('g.relphi-marker-unit') !== vector.parentNode) {
        return;
      } else {
        node.remove();
      }
  ...
  function ensurePlutoVector(group, unit, cx, cy, markerColor) {
    ...
    if (!vector) {
      ...
      vector.innerHTML = [
        '<circle cx="0" cy="-6" r="3.1"/>',
        '<path d="M -7 -1.5 Q 0 5.2 7 -1.5"/>',
        '<line x1="0" y1="4" x2="0" y2="11"/>',
        '<line x1="-5" y1="8" x2="5" y2="8"/>'
      ].join('');
      unit.appendChild(vector);
    }
    removeCompetingPlutoMarks(group, vector);
    vector.setAttribute('transform', `translate(${cx.toFixed(2)} ${(cy - 0.2).toFixed(2)})`);
    vector.setAttribute('fill', 'none');
    vector.setAttribute('stroke', markerColor);
    vector.setAttribute('color', markerColor);
    vector.setAttribute('stroke-width', '1.8');
    vector.setAttribute('stroke-linecap', 'round');
    vector.setAttribute('stroke-linejoin', 'round');
    vector.querySelectorAll('*').forEach(function (node) {
      node.setAttribute('stroke', markerColor);
  ...
  function styleGlyph(group, unit, knob, text) {
    ...
    if (isPluto) {
      ...
      ensurePlutoVector(group, unit, cx, cy, markerColor);
    } else {
