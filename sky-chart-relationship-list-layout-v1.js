...
(function () {
  ...
  if (window.__relphiSkyRelationshipListLayoutV25) return;
  window.__relphiSkyRelationshipListLayoutV25 = true;

  const NS = 'http://www.w3.org/2000/svg';
  const STYLE_ID = 'skyRelationshipListLayoutV25';
  const OWNER = 'relationship-layout-v25';
  const MASTER_VIEWBOX = '-32 -32 64 64';
  const MASTER_RADIUS = 19;
  // The comparison wheel uses radius 19, so 38px is the matching nominal diameter.
  ...
  function installStyle() {
    ...
    ['skyRelationshipListLayoutV24','skyRelationshipListLayoutV23','skyRelationshipListLayoutV22','skyRelationshipListLayoutV21','skyRelationshipListLayoutV20','skyRelationshipListLayoutV19','skyRelationshipListLayoutV18','skyRelationshipListLayoutV17','skyRelationshipListLayoutV16']
    ...
    style.textContent = `
      .sky-foundation-relationship-row{
        ...
        position:relative;
        display:grid;
        grid-template-columns:${GLYPH_DISPLAY_SIZE}px minmax(82px,1fr) 46px ${GLYPH_DISPLAY_SIZE}px minmax(82px,1fr);
        grid-template-areas:
      ...
      @media(max-width:620px){
        .sky-foundation-relationship-row{
          grid-template-columns:${GLYPH_DISPLAY_SIZE}px minmax(76px,1fr) 44px ${GLYPH_DISPLAY_SIZE}px minmax(76px,1fr);
          column-gap:4px;
          min-height:58px;
          padding:4px 7px 3px 12px;
        }
        .sky-foundation-relationship-copy small{gap:3px;font-size:.66rem}
      }
    `;
  ...
  function composeStructure(row) {
    ...
    row.dataset.relationshipLayout = 'v25';
  }

  function paintRowGlyphs(row) {
    if (!(row instanceof HTMLElement) || row.dataset.relationshipPainted === 'v25') return;
    const aspect = String(row.dataset.aspect || '').toLowerCase();
    ...
    row.dataset.relationshipPainted = 'v25';
  }

  function registerRow(row) {
    if (!(row instanceof HTMLElement)) return;
    composeStructure(row);
    if (row.dataset.relationshipPainted === 'v25') return;
    if (visibleRows) visibleRows.observe(row);
    else paintRowGlyphs(row);
  }

  function registerRows(root) {
    root?.querySelectorAll?.('.sky-foundation-relationship-row').forEach(registerRow);
  }
  ...
  function start() {
    ...
    ].forEach(name => window.addEventListener(name, scheduleBind));
  }
