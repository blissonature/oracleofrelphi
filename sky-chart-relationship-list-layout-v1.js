...
(function () {
  ...
  if (window.__relphiSkyRelationshipListLayoutV7) return;
  window.__relphiSkyRelationshipListLayoutV7 = true;

  const NS = 'http://www.w3.org/2000/svg';
  const STYLE_ID = 'skyRelationshipListLayoutV7';
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      .sky-relationship-master-stage{display:block;width:34px!important;height:34px!important;max-width:34px!important;max-height:34px!important;overflow:visible;align-self:center;color:inherit}
  ...
  async function renderMaster(node, id, color, circled) {
    ...
    if (!entry || !component?.createBubble) throw new Error('Master Glyph List entry unavailable: ' + id);
  ...
  function replaceWithMasterGlyphs(row, color) {
    if (row.dataset.relationshipMasterGlyphs === 'v7') return;
    const current = Array.from(row.querySelectorAll(':scope > svg'));
    ...
    row.dataset.relationshipMasterGlyphs = 'v7';
  }
  ...
  function compose(row) {
    ...
    if (row.dataset.relationshipLayout === 'v7') return;
    ...
    row.dataset.relationshipLayout = 'v7';
  }
