...
(function(){
  ...
  const ASPECT_ANGLES={conjunction:0,'semi-sextile':30,octile:45,sextile:60,quintile:72,square:90,trine:120,'tri-octile':135,'bi-quintile':144,quincunx:150,opposition:180};
  ...
  let mount=null,selectedIndex=null,initialRenderAttempts=0;
  ...
  function orbLabel(value){let minutes=Math.round(Number(value)*60),degrees=Math.floor(minutes/60);minutes%=60;return `${degrees}°${String(minutes).padStart(2,'0')}′`}
  ...
  function point(longitude,radius=56){const angle=(norm(Number(longitude))-180)*Math.PI/180;return{x:90+radius*Math.cos(angle),y:90+radius*Math.sin(angle)}}
  function diagramMarkup(relation,pA,pB){
    const a=point(relation.left.value),b=point(relation.right.value);
    return `<div class="sky-selected-aspect-diagram" aria-label="Isolated aspect diagram"><svg viewBox="0 0 180 180" role="img"><circle class="sky-selected-aspect-orbit" cx="90" cy="90" r="56"></circle><line class="sky-selected-aspect-radius" x1="90" y1="90" x2="${a.x.toFixed(2)}" y2="${a.y.toFixed(2)}"></line><line class="sky-selected-aspect-radius" x1="90" y1="90" x2="${b.x.toFixed(2)}" y2="${b.y.toFixed(2)}"></line><circle class="sky-selected-aspect-center" cx="90" cy="90" r="4"></circle><circle class="sky-selected-aspect-point sky-a" cx="${a.x.toFixed(2)}" cy="${a.y.toFixed(2)}" r="7"></circle><circle class="sky-selected-aspect-point sky-b" cx="${b.x.toFixed(2)}" cy="${b.y.toFixed(2)}" r="7"></circle></svg><h4>${esc(aspectLabel(relation))}</h4><p class="sky-selected-aspect-angle">Approximately ${ASPECT_ANGLES[relation.aspect.id]}°</p><p class="sky-selected-aspect-label sky-a"><span></span>Sky A · ${esc(name(relation.left))} ${esc(pA.label)}</p><p class="sky-selected-aspect-label sky-b"><span></span>Sky B · ${esc(name(relation.right))} ${esc(pB.label)}</p><strong>Orb ${orbLabel(relation.orb)}</strong></div>`;
  }
  ...
  function draw(target,id,options){
    ...
    if(!entry||!component?.mount){target?.replaceChildren();if(target){target.classList.add('sky-selected-canonical-error');target.dataset.missingCanonicalGlyph=id||'unknown'}return}
    try{component.mount(target,entry.id,options)}catch(_){target.classList.add('sky-selected-canonical-error');target.dataset.missingCanonicalGlyph=entry.id}
  }
  function markSelected(index){const active=Number.isInteger(index);document.querySelectorAll('.sky-foundation-relationship-row[data-relation-index]').forEach(row=>active?row.setAttribute('aria-current',Number(row.dataset.relationIndex)===index?'true':'false'):row.removeAttribute('aria-current'));document.querySelectorAll('.sky-foundation-aspect[data-relation-index]').forEach(line=>active?line.dataset.selectedRelation=Number(line.dataset.relationIndex)===index?'true':'false':delete line.dataset.selectedRelation)}
  function clearSelection(source='white-space'){
    selectedIndex=null;markSelected(null);const panel=ensureMount();if(panel){panel.hidden=true;delete panel.dataset.relationIndex;panel.dataset.selectionSource=source}
    requestAnimationFrame(()=>{if(selectedIndex==null)markSelected(null)});
    window.dispatchEvent(new CustomEvent('relphi:selected-relationship-cleared',{detail:{source}}));
  }
  function cardMarkup(slot,record,card){const p=position(record);return `<article class="sky-selected-card" data-selected-card="${slot}" data-card-title="${esc(card.title)}" data-card-decan="${card.decan}"><p class="sky-selected-card-label">Sky ${slot} · ${esc(name(record))}</p><img src="${esc(card.image)}" alt="${esc(card.title)} card art"><h4>${esc(card.title)}</h4><p>${esc(name(record))} in ${esc(card.sign)}<br>${esc(p.label)} · H${record.house}<br>${esc(card.sign)} · decan ${card.decan}</p></article>`}
  ...
  async function render(index,initiator){
    ...
    selectedIndex=index;markSelected(index);panel.hidden=false;panel.dataset.relationIndex=String(index);panel.dataset.selectionSource=initiator==='foundation-rerender'&&panel.dataset.selectionSource==='initial-relationship'?'initial-relationship':initiator;
    ...
    body.innerHTML=`<div class="sky-selected-graphic"><svg viewBox="0 0 360 130" role="img" aria-label="${esc(name(relation.left))} ${esc(label)} ${esc(name(relation.right))}"><line class="sky-selected-graphic-line" x1="78" y1="65" x2="282" y2="65" stroke="${relation.aspect.color}"></line><g data-selected-graphic-a transform="translate(78 65)"></g><g data-selected-graphic-aspect transform="translate(180 65)"></g><g data-selected-graphic-b transform="translate(282 65)"></g></svg></div><header class="sky-selected-facts"><h3>${esc(name(relation.left))} ${esc(label)} ${esc(name(relation.right))}</h3><p><span>Sky A · ${esc(name(relation.left))} at ${esc(pA.label)} · ${HOUSE_NAMES[relation.left.house-1]} house</span><span>Sky B · ${esc(name(relation.right))} at ${esc(pB.label)} · ${HOUSE_NAMES[relation.right.house-1]} house</span><strong>Orb: ${orbLabel(relation.orb)}</strong></p></header><div class="sky-selected-cards">${cardMarkup('A',relation.left,cardA)}${diagramMarkup(relation,pA,pB)}${cardMarkup('B',relation.right,cardB)}</div><section class="sky-selected-progressive" aria-label="Progressive relationship interpretation"><details class="sky-selected-reveal" data-reveal-level="symbol"><summary>1 · See the relationship</summary><div class="sky-selected-reveal-content"><span class="sky-selected-reveal-glyph"><span data-selected-reveal-glyph aria-hidden="true"></span></span><strong>${esc(label)}</strong><p>${esc(copy.meaning)}</p></div></details><details class="sky-selected-reveal" data-reveal-level="cards"><summary>2 · Read the two cards</summary><div class="sky-selected-reveal-content"><p>${esc(copy.cards)}</p></div></details><details class="sky-selected-reveal" data-reveal-level="synthesis"><summary>3 · Bring the relationship together</summary><div class="sky-selected-reveal-content"><p>${esc(copy.synthesis)}</p></div></details></section>`;
    const aspectWheel=body.querySelector('.sky-selected-aspect-diagram svg');if(aspectWheel)aspectWheel.dataset.zodiacOrigin='aries-0-at-9';
    const graphic=body.querySelector('.sky-selected-graphic svg');
    [draw(graphic.querySelector('[data-selected-graphic-a]'),relation.left.id,{size:40,circle:true,color:COLORS.A}),draw(graphic.querySelector('[data-selected-graphic-aspect]'),relation.aspect.id,{size:36,circle:true,color:relation.aspect.color}),draw(graphic.querySelector('[data-selected-graphic-b]'),relation.right.id,{size:40,circle:true,color:COLORS.B}),draw(body.querySelector('[data-selected-reveal-glyph]'),relation.aspect.id,{size:30,circle:false,color:relation.aspect.color,label:false})];
    if(initiator!=='initial-relationship')panel.scrollIntoView({behavior:'smooth',block:'nearest'});window.dispatchEvent(new CustomEvent('relphi:selected-relationship-rendered',{detail:{index,relation,source:initiator}}));
  }
  ...
  window.addEventListener('relphi:sky-foundation-clear-selection',event=>clearSelection(event.detail?.source||'white-space'));
  function onReady(force=false){
    ...
    if(selectedIndex!=null&&relationshipRow(selectedIndex)){if(force)render(selectedIndex,'foundation-rerender');return}
    const first=Array.from(document.querySelectorAll('.sky-foundation-relationship-row[data-relation-index]')).find(row=>!row.hidden),index=indexFrom(first);
    if(index!=null){initialRenderAttempts=0;render(index,'initial-relationship');return}
    if(initialRenderAttempts<100){initialRenderAttempts+=1;setTimeout(()=>onReady(false),100)}
  }
  window.addEventListener('relphi:sky-foundation-ready',()=>onReady(true));if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>onReady(false),0),{once:true});else setTimeout(()=>onReady(false),0);
})();
