...
(function () {
  ...
  function readyForCalculation() { return !!(byId('skyCalcDateTime')?.value && byId('skyCalcLatitude')?.value && byId('skyCalcLongitude')?.value); }
  function setProgress(message) {
    let note = byId('relphiHereNowProgress');
    const stage = byId('relphiCalculateStage');
    if (!stage) return;
    if (!note) { note = document.createElement('p'); note.id = 'relphiHereNowProgress'; note.className = 'generated-note'; note.setAttribute('aria-live', 'polite'); stage.appendChild(note); }
  ...
  function hideManualCalculator() {
    ...
    [calculator, drawer].forEach(function (node) { if (!node) return; node.open = false; node.hidden = true; node.setAttribute('hidden', ''); node.style.removeProperty('display'); });
  ...
  function runHereNow() {
    ...
    (function waitForLocation() {
      ...
      if (Date.now() - started > 12000) { setProgress('Current location could not be resolved. Choose a time and place instead.'); return; }
