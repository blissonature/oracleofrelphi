(function () {
  ...
  function drawRelphiExportBrand(ctx, canvas, height = 54, logoImage = null) {
    ...
    const pad=12;
    const logoSize=Math.min(34,h-16);
    ctx.save();
    ...
    if (logoImage) ctx.drawImage(logoImage,pad,y+(h-logoSize)/2,logoSize,logoSize);
    const textX=pad+(logoImage ? logoSize+10 : 0);
    ctx.textAlign='left';
    ctx.textBaseline='middle';
    ctx.font='800 13px Montserrat, Arial, sans-serif';
    ctx.fillStyle='#111';
    const prefix='Oracle of ';
    ctx.fillText(prefix,textX,y+h/2-7);
    const prefixW=ctx.measureText(prefix).width;
    ctx.fillStyle='#dc1f18';
    ctx.fillText('Relphi',textX+prefixW,y+h/2-7);
    ctx.fillStyle='#5f5751';
    ctx.font='700 10px Montserrat, Arial, sans-serif';
    ctx.fillText('Drawing Board · oracleofrelphi.com',textX,y+h/2+10);
    ctx.restore();
  }
  ...
  async function downloadShortListReadingHtml({ includeArt = true } = {}) {
    ...
    const brandLogoSource = await imageDataUrlForExport(RELPHI_EXPORT_BRAND.logo);
    const createdAt = new Date();
  ...
  async function saveCardRowImage(dialog) {
    ...
    const [brandLogoImage, images] = await Promise.all([
      ...
      Promise.all(items.map(card => loadImage(rwsExportImagePath(card)).catch(() => null)))
    ]);
  ...
  function bindRenderedDrawingBoardActions(wrap) {
    if (!wrap) return;
    const bind = (id, handler) => {
      const button = wrap.querySelector('#' + id);
      if (!button) return;
    ...
    bind('drawRandomRowCard', () => drawRandomRowCard());
  ...
  function renderShortList() {
    ...
    if (tableColorInput) tableColorInput.addEventListener('input', () => { state.rowTableColor = tableColorInput.value || '#7d1f28'; const board = wrap.querySelector('.card-row-board'); if (board) board.style.setProperty('--row-table-bg', state.rowTableColor); });
    if (tableColorInput) tableColorInput.addEventListener('change', () => { state.rowTableColor = tableColorInput.value || '#7d1f28'; renderShortList(); });
