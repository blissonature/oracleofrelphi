(function () {
  ...
  function renderShortList() {
    ...
    wrap.innerHTML = `<details class="short-list-drawer card-row-drawing-board"><summary><strong>Drawing Board <span class="card-row-count">${items.length}</span></strong></summary><span class="short-list-actions card-row-icon-toolbar card-row-action-staging" aria-label="Drawing Board quick actions" hidden><button type="button" id="drawRandomRowCard" title="Draw random card" aria-label="Draw random card">Draw</button><button type="button" id="addCardPlaceholder" title="Add placeholder" aria-label="Add placeholder">Add placeholder</button><label class="quick-reversal-toggle" title="Allow reversed cards in future draws"><input id="rowAllowReversalsQuick" type="checkbox" ${state.rowAllowReversals ? 'checked' : ''}> Reversals</label><button type="button" id="undoShortList" ${state.shortListUndo.length ? '' : 'disabled'} title="Undo" aria-label="Undo">Undo</button><button type="button" id="redoShortList" ${state.shortListRedo.length ? '' : 'disabled'} title="Redo" aria-label="Redo">Redo</button><button type="button" id="clearShortList" ${displaySlots ? '' : 'disabled'} title="Clear board" aria-label="Clear Drawing Board">Clear</button></span>${moreOptionsHtml}${boardHtml}</details>`;
