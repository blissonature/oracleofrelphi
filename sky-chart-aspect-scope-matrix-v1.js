...
function setCells(scope,aspect,checked){
  cells(scope,aspect).forEach(([scopeId,aspectId])=>checked?state[scopeId].add(aspectId):state[scopeId].delete(aspectId));
}
function choice(scope,aspect,labelText,summary=false){
  ...
  input.type='checkbox';
  input.dataset.aspectMatrixScope=scope;
  input.dataset.aspectMatrixAspect=aspect;
...
function matrixRow(aspect,labelText,master=false){
  ...
  label.className='sky-chart-aspect-list-label';
  label.textContent=labelText;
  const choices=document.createElement('div');
  choices.className='sky-chart-aspect-list-choices';
  choices.setAttribute('role','group');
  choices.setAttribute('aria-label',labelText);
  choices.append(
    choice('all',aspect,`${labelText}: all relationship scopes`),
    ...SCOPES.map(scope=>choice(scope.id,aspect,`${labelText}: ${scope.label}`))
  );
...
function renderPopover(){
  ...
  list.className='sky-chart-aspect-list sky-chart-aspect-matrix-list';
  list.dataset.aspectList='matrix';
  ...
  header.append(title,cols);
  list.append(header,matrixRow('all','All aspects',true));
  ASPECTS.forEach(aspect=>list.appendChild(matrixRow(aspect.id,aspect.label,false)));
...
function renderClosedSummary(owner){
  ...
  if(!summary){
    ...
    summary.className='sky-chart-aspect-summary-choices';
    summary.setAttribute('role','group');
    summary.setAttribute('aria-label','All aspects by relationship scope');
    summary.append(
      choice('all','all','All aspects in all relationship scopes',true),
      ...SCOPES.map(scope=>choice(scope.id,'all',`All aspects in ${scope.label}`,true))
    );
    const toggle=head.querySelector('[data-aspect-filter-toggle]');
    head.insertBefore(summary,toggle||null);
  }
}
function updateInputs(){
  document.querySelectorAll('[data-aspect-matrix-scope][data-aspect-matrix-aspect]').forEach(input=>{
    const scope=input.dataset.aspectMatrixScope,aspect=input.dataset.aspectMatrixAspect;
    const current=cellState(scope,aspect);
    input.checked=current.checked;
    input.indeterminate=current.indeterminate;
    input.disabled=current.available===0;
  });
}
...
function applyMatrix({announce=true}={}){
  ...
  document.querySelectorAll('.sky-foundation-relationship-row,[data-layer="aspects"]>.sky-foundation-aspect').forEach(node=>{
    node.classList.toggle('sky-chart-aspect-multiselect-hidden',!visible(node));
  });
  updateInputs();
...
function ensureUI(){
  ...
  renderClosedSummary(owner);
  const list=popover()?.querySelector('[data-aspect-list="matrix"]');
  if(!list)renderPopover();
  updateInputs();
  return true;
}
function handleChange(event){
  ...
  if(!input)return;
  setCells(input.dataset.aspectMatrixScope,input.dataset.aspectMatrixAspect,input.checked);
  applyMatrix();
}
function refresh(){
  queued=false;
  if(!ensureUI())return;
  applyMatrix({announce:false});
}
